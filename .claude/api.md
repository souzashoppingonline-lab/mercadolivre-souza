...
| `GET /api/bi/margem?days&store_id` | **Inteligência de Margem** (BI) — motor determinístico, sem LLM (`computarMargem()` em `routes/bi.js`): agrega `/vendas/detalhado` por `item_id` e devolve `{periodo, resumo, produtos[], bandas_margem[], concentracao, frete_tarifa:{por_logistica,por_conta}, acoes[]}`. Cada produto traz faturamento/qtd/margem/MC%/frete%/tarifa%/classificação (ESTRELA/VOLUME_BAIXA_MARGEM/ALTA_MARGEM_BAIXO_VOLUME/DESTRUIDOR_MARGEM/PREJUIZO/NEUTRO)/estoque+ruptura (mesma fórmula de `getRupturaEstoque`)/score 0-100 com breakdown/**`amostra_pequena`** (bool, < 3 pedidos no período)/**`tendencia_semanal[]`** (até 6 semanas de `{semana, mc_pct, faturamento}`, janela fixa de 42 dias independente de `days`). `resumo.causa_variacao` = decomposição EXATA da variação de margem vs. período anterior (`componentes[]` de faturamento/custo/imposto/tarifa/frete comprador/frete vendedor, soma bate com `delta_margem`) + `mix` (% do faturamento em SKUs com MC abaixo do limiar, atual vs. anterior). `acoes` é o ranking Top 10 por impacto estimado, cada uma com problema/evidência/ação/esforço/confiança/**premissa explícita**; ações `REPRECIFICAR` trazem `cenarios[]` (+3%/+5%/+10%, cada um com impacto estimado próprio). Limiares e fórmulas completos em `business-rules.md`. Módulo `bi` (só admin) |
| `POST /api/bi/margem/narrativa` `{days, store_id}` | **Inteligência de Margem — Fase 2 (IA sob demanda)**: reusa `computarMargem()` e manda um contexto REDUZIDO (resumo + produtos críticos/estrela/ruptura + top ações, nunca o portfólio inteiro) pro Haiku (`server/src/ai/margemNarrativa.js`), que só INTERPRETA em texto — nunca recalcula margem. Devolve `{ok, narrativa:{resumo_executivo, causa_raiz, o_que_eu_faria_agora[]}, periodo}`. 503 se `ANTHROPIC_API_KEY` não configurada (`{error}`). Sem persistência — cada chamada gera uma análise nova. Módulo `bi` (só admin) |
