...
| `GET /api/conciliacao/pagamentos/:paymentId` | 1 pagamento completo (`SELECT p.*` + `buyer_nickname`/`title`/`order_total_amount`/`item_id`/`store_nickname` via JOIN) — usado pelo modal de detalhe da grid, inclui `raw_data` (payload bruto de `/collections/:id`) pra auditoria |
