...
| `POST /api/vendas/:orderId/atualizar` | reconsulta **1 pedido** na API do ML na hora (`/orders/:id`, `/shipments/:id/costs`, `/collections/:id`) e grava — mesma lógica de `handleOrder`/`handleShipment`/`handlePayment` do worker, replicada aqui (não importa `worker.js`, que inicia filas/crons ao carregar). Ação pontual (regra de arquitetura #3), não leitura de listagem. Devolve `{row, shipping_error, payment_error}` — `row` no mesmo formato de uma linha de `/vendas/detalhado` (mesmo SELECT/fórmula, extraídos pra `VENDA_DETALHE_SELECT`/`calcularMargemLinha`). Usado pelo botão 🔄 do card "Resumo por Venda" (BI, ver `modules.md`) **e** pela coluna Frete Vend. da tabela "Detalhamento por Venda" em `pages/vendas.html` (Operacional) — quando a venda é recente demais pra Conciliação/webhook já terem chegado |
