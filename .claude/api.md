...
| Rota | Descrição |
|---|---|
| `GET /api/usuarios` | **v80, só admin:** lista os usuários de acesso restrito (`id`, `username`, `role`, `created_at` — **nunca** `password_hash`) + `gate_ativo` (estado de `STAFF_AUTH_ENABLED`), `atual` (quem está logado) e `papeis`. Ver `auth-staff.md` |
| `POST /api/usuarios` `{username, password, role}` | **v80, só admin:** cria usuário. Valida nome `^[a-z0-9._-]{3,32}$` (normalizado pra minúsculo), senha ≥ 6, papel `admin\|embalagem\|shopee-demo`, nome único. Senha em bcrypt (custo 10) |
| `PATCH /api/usuarios/:id` `{password?, role?}` | **v80, só admin:** troca senha e/ou papel. Bloqueia rebaixar o próprio usuário e tirar o papel do último admin |
| `DELETE /api/usuarios/:id` | **v80, só admin:** exclui. Bloqueia excluir a si mesmo e o último admin. **Não** encerra a sessão aberta do usuário (JWT stateless — ver `known-bugs.md`) |
| `GET /api/ranking/ads?marketplace&fase&store_id&ciclo` | anúncios em rankeamento com stats (`sales_count`, `dias`, `ritmo_dia`, `faturamento` **acumulado**, `estoque_atual`/`preco_atual`, `last_*`, `marketplace`, `fase`, `ciclo`, `campanha_nome`, `roas`/`orcamento_diario`, `preco_anterior`/`preco_atual`, `ciclos_anteriores`) + `sugerir_ranqueado` (bool). **v80 (todas as fases):** `visitas_dia`, `qualidade` e `buybox` — os `last_*` com **fallback** para `item_visits` (média 7d), `item_seo_score.score` e `catalog_competition.winner_item_id` quando o snapshot ainda não rodou naquela fase (o card usa estes, não os `last_*` crus). **v80 (fase recuperação):** `dias_sem_venda`, `semaforo` (observando/atencao/decisao), `nunca_vendeu`, `visitas_dia`, `visitas_media_7d`, `vendas_na_fase` (vendas desde `recuperacao_started_at`), `diagnostico {tipo,titulo,texto,acao}`, `intervencoes[]` (3 últimas notas tipadas, cada uma com `efeito`), e os campos de qualidade vindos de `item_seo_score`/`catalog_competition` (`conversion_rate`, `seo_score`, `pictures_count`, `has_video`, `title_length`, `description_word_count`, `required_attrs_missing`, `missing_required_attrs`, `is_full`, `price_to_win`). Filtros opcionais `marketplace` (ML/SHOPEE), `fase` (rankeando/ranqueado/monitoramento/recuperacao), `store_id` (empresa/loja) e `ciclo` (número) |
...
| `PATCH /api/ranking/ads/:id` `{active?, milestone_every?, fase?, campanha_nome?, roas?, orcamento_diario?, preco_anterior?, preco_atual?, ads_investido?}` | pausar/retomar, mudar intervalo do marco, mudar de **fase** (`rankeando`/`ranqueado`/`monitoramento`/`recuperacao`; `ranqueado` carimba `ranqueado_em`, `rankeando` limpa, `monitoramento` carimba `monitoramento_started_at`, `recuperacao` carimba `recuperacao_started_at` — v80), ou salvar os campos **manuais** do ciclo (nome da campanha + ROAS/orçamento + preço anterior/atual; `''`/null zera a coluna) |
...
| `POST /api/ranking/ads/:id/precos` `{de?, para}` | **v88:** lança manualmente uma entrada no histórico de preço (botão "Lançar no histórico" do bloco de campanha) — grava direto em `ranking_events` (`event_type:'preco'`, `detail:{de,para,manual:true}`), **sem** `ranking.emit()` (sem WS/Telegram — é o usuário documentando o preço da campanha, não uma mudança real detectada pelo webhook). `para` obrigatório (400 sem ele), `de` opcional. Aparece na mesma listagem de `GET .../precos` |
...
| `GET /api/ranking/ads/:id/notas` | **v76:** log de alterações/anotações (mais recente primeiro). `/ads` devolve `notas_count`. **v80:** cada nota vem com `tipo`, `baseline` e `efeito` — o efeito é **calculado na leitura** (atual − baseline): `{veredito: medindo\|funcionou\|parcial\|piorou\|sem_efeito, dias, faltam?, vendas, visitas_de, visitas_para, visitas_pct}` |
| `POST /api/ranking/ads/:id/notas` `{texto, tipo?}` | **v76:** registra uma anotação livre. **v80:** com `tipo` (`titulo\|keywords\|fotos\|descricao\|preco\|ads\|atributos\|frete\|outro`) vira **intervenção medida** — carimba `baseline` com visitas/conversão/score/vendas/preço do momento. Sem `tipo`, continua anotação livre (sem baseline) |
...
| `POST /api/ranking/fase-lote` `{item_ids:[...]}` | Estágio atual (bulk) de vários anúncios de uma vez — `{fases: {item_id: {fase, ranking_ad_id}}}`, só os `item_ids` rastreados no módulo aparecem no objeto. Usado pela tag de estágio em `bi-vendas.html` e pelo agregado de `/api/bi/rankeamento`. Reusa `ranking.buscarFasePorItemIds()` (mesmo join usado nos dois lugares — nunca duplicado). Considera `ranking_ad_links` (vínculo catálogo↔tradicional), não só `ranking_ads.ml_id` direto. Sem gate de módulo específico (cai em `operacional`, `roles:'*'`, igual ao resto de `/api/ranking`) |
...
| `GET /api/bi/margem/acoes-status` | Fase F — devolve todo o conteúdo de `business_insights` (`{status:[{item_id,tipo,status,nota,updated_by,updated_at,concluida_em}]}`). Tabela pequena (1 linha por ação já vista), sem paginação; o cliente casa localmente com `acoes[]` de `/margem` por `(item_id,tipo)` |
| `PATCH /api/bi/margem/acoes-status` | Upsert do status de UMA ação (`{item_id,tipo,status,nota}` → `status` ∈ `pendente/em_andamento/concluida/descartada`, 400 se fora da lista). `updated_by` vem de `req.staffUser.username` (JWT do login de funcionário). **v87**: grava `concluida_em` (carimbo, não valor calculado) só na transição PRA `'concluida'`; sai `NULL` ao mover pra qualquer outro status. Nunca infere resultado real diretamente aqui — ver `GET /margem/acoes-feedback` abaixo e `business-rules.md` |
| `GET /api/bi/margem/acoes-feedback` | **v87**: efeito OBSERVADO (nunca causal) das ações `concluida` com `concluida_em` gravado — `{feedback:[{item_id,tipo,concluida_em,dias_desde_conclusao,antes:{pedidos,faturamento,margem,mc_pct,faturamento_dia,margem_dia},depois:{...mesmos campos}}]}`. "Antes" = 14 dias fechados antes de `concluida_em`; "depois" = desde `concluida_em` até agora — mesmo padrão de `recuperacao_antes_depois` (`rankeamento.md`). 1 request em lote pra tela toda (não N por ação); reusa `VENDA_DETALHE_SELECT`/`calcularMargemLinha`. Módulo `bi` (só admin) |

## Agente Financeiro (`routes/agenteFinanceiro.js`, montado em `/api/bi/agente-financeiro`, **só admin** — mesmo gate do módulo `bi`, ver `modules.md`)
4 relatórios sob demanda (botão, não chat) que cruzam Postgres operacional + Supabase Financeiro. Cada rota devolve `{ dados, interpretacao, ia_erro }`: `dados` é sempre o cálculo determinístico (`server/src/financeiroCalc.js` ou `computarMargem` de `routes/bi.js`); `interpretacao` é o texto da IA (`server/src/ai/agenteFinanceiro.js`, formato `{resumo, pontos_de_atencao[], recomendacao}`) — `null` se `?ia=0` ou se a IA falhar (nesse caso `ia_erro` traz a mensagem real; os `dados` continuam válidos, a rota nunca derruba o relatório por causa da IA). 503 se o Supabase Financeiro não estiver configurado (`SUPABASE_FIN_URL`/`SUPABASE_FIN_KEY`, mesmo erro do módulo Financeiro).
| Rota | Descrição |
|---|---|
| `GET /agente-financeiro/dre?mes&ano&ia` | DRE do mês (default: mês atual) + comparativo com o mês anterior + ponto de equilíbrio. Lê `sales_entries`+`expenses` (Supabase) — mesma fórmula de `financeiro-despesas.html` (`renderDre`/`renderBe`), extraída pra `financeiroCalc.getDRE` |
| `GET /agente-financeiro/fluxo?dias&ia` | Fluxo de caixa + projeção de N dias (1-60, default 30): caixa atual, entradas/saídas previstas, saldo projetado, dia em que fica negativo (se ficar). Lê `cash_flow_entries`+`receivables`+`boletos_mensais`+`parcelas_cartao`+`cash_flow_recurring` — mesma fórmula de `financeiro-projecao-caixa.html` (`buildDias`), sem o ajuste de tendência manual da tela (sempre 0%) |
| `GET /agente-financeiro/contas-a-pagar?ia` | Boletos + faturas de cartão pendentes: total, vencidos, próximos 7d/30d, por empresa |
| `GET /agente-financeiro/cruzamento-ml?days&store_id&ia` | Compara a margem REAL das vendas ML no período (`computarMargem`, Postgres — mesma função da Inteligência de Margem) com o que foi lançado em `sales_entries` no MESMO período (`ml.periodo.de/ate`). Os dois bancos não se joinam em SQL — o cruzamento é feito na aplicação |
...
| `GET /api/pedidos/:id/detalhes` | pedido completo com cálculo de margem (custo, imposto, tarifa, fretes). Traz `fonte_taxa`/`fonte_frete` (precedências independentes, v86) e `freteMotoboyAplicado` (bool) quando o custo motoboy Flex (v85) substituiu o frete do vendedor nesta venda |
...
| `GET /api/vendas/detalhado?store_id&status&days&search&date_from&date_to` | linha a linha com margem calculada por pedido — fórmula/precedência da taxa em `finance.md`. Tarifa/frete vendedor por pedido em 3 níveis (`LEFT JOIN LATERAL` por `order_id`): **1)** Conciliação (`mp_account_movements`, separa `mp_fee_amount`/`shipping_fee_amount`); **2)** `ml_payments` tempo real (`transaction_amount−net_received_amount`, tudo na tarifa, frete=0); **3)** `orders.ml_fee`/`shipping_seller_cost`. Cada linha traz `fonte_taxa` (`conciliacao`\|`pagamento`\|`pedido`\|`manual`) e `fonte_frete` (mesmos valores, precedência PRÓPRIA e independente — v86) e `tem_conciliacao`; `summary` traz `pedidos_conciliados` (cobertura de taxa real). Linha traz `freteMotoboyAplicado` (bool) quando o custo motoboy Flex (v85) substituiu o frete do vendedor nesta venda — ver `business-rules.md`. Também traz `buyer_nickname`, `buyer_first`/`buyer_last` (nome real do comprador, de `orders.raw_data->buyer` — nem sempre vem preenchido), `pack_id` (nº do carrinho, de `raw_data`), `sku` (best-effort: `seller_sku` do item do pedido, senão `seller_custom_field`), `thumbnail`/`estoque_atual` (join com `items` — estoque é o **atual**, não o saldo no momento da venda). Consumido também pelo card "Resumo por Venda" do módulo BI, ver `modules.md` |
| `POST /api/vendas/:orderId/atualizar` | **v82: casca fina** em volta de `financeService.reconciliarPedido()` (`server/src/financeService.js`) — reconsulta **1 pedido** na API do ML na hora (`/orders/:id`, `/shipments/:id/costs`, `/collections/:id`) e grava. **O mesmo serviço é usado pelo `financeReconciliationJob`** (worker, a cada 10min — ver `workers.md`/`business-rules.md`), nunca duas lógicas de reconciliação. Ação pontual (regra de arquitetura #3), não leitura de listagem. Devolve `{ok, row, shipping_error, payment_error, finance_synced}` — `row` no mesmo formato de uma linha de `/vendas/detalhado` (mesmo SELECT/fórmula, em `server/src/vendaMargem.js`). Usado pelo botão 🔄 do card "Resumo por Venda" (BI, ver `modules.md`) **e** pela coluna Frete Vend. da tabela "Detalhamento por Venda" em `pages/vendas.html` (Operacional) — quando a venda é recente demais pra Conciliação/webhook já terem chegado |
| `PATCH /api/vendas/:orderId/tarifa { tarifa }` | **v84**: edição manual de tarifa por pedido — escape hatch pra quando a Conciliação vem errada e `CONCILIACAO_TARIFA_LATERAL` não pega o caso (ver `business-rules.md`/`known-bugs.md`). `tarifa` número ≥0 grava `orders.tarifa_manual` (maior precedência de todas as fontes); `tarifa: null` remove a edição e volta pro cálculo automático. `tarifa_manual_by`/`tarifa_manual_at` gravam quem editou (`req.staffUser.username`). Devolve `{ok, row}` no mesmo formato de `/vendas/detalhado` (recalculado, nunca no cliente). Botão ✏️ no campo Tarifa do card "Resumo por Venda" (BI) |
| `PATCH /api/vendas/:orderId/frete { frete_vendedor }` | **v86**: mesmo escape hatch acima, pro frete do vendedor — pedido explícito do usuário. `frete_vendedor` número ≥0 grava `orders.frete_vendedor_manual` (vence Conciliação/`ml_payments`/`shipping_seller_cost`/substituição do custo motoboy Flex); `frete_vendedor: null` remove a edição. Precedência PRÓPRIA, independente da tarifa (editar um não mexe no outro). `frete_vendedor_manual_by`/`_at` gravam quem editou. Devolve `{ok, row}` recalculado. Botão ✏️ no campo "Frete vend." do card "Resumo por Venda" (BI) |
...
| `GET /api/bi/margem?days\|(date_from&date_to)&store_id&category_id` | **Inteligência de Margem** (BI) — motor determinístico, sem LLM (`computarMargem()` em `routes/bi.js`): agrega `/vendas/detalhado` por `item_id` e devolve `{periodo, resumo, saude, mudancas, produtos[], bandas_margem[], concentracao, frete_tarifa:{por_logistica,por_conta}, acoes[]}`. Período: **ou** `days` (N dias terminando hoje, padrão 30) **ou** `date_from`/`date_to` explícitos (YYYY-MM-DD — Hoje/Ontem/Mês atual/Mês anterior/personalizado, resolvidos no frontend); qualquer um dos dois sempre compara com um período ANTERIOR de duração IGUAL, imediatamente antes. `category_id` opcional filtra por `items.category_id`. Cada produto traz faturamento/qtd/margem/MC%/frete%/tarifa%/classificação (ESTRELA/VOLUME_BAIXA_MARGEM/ALTA_MARGEM_BAIXO_VOLUME/DESTRUIDOR_MARGEM/PREJUIZO/NEUTRO)/estoque+ruptura (mesma fórmula de `getRupturaEstoque`)/score 0-100 com breakdown/**`amostra_pequena`** (bool, < 3 pedidos no período)/**`tendencia_semanal[]`** (até 6 semanas de `{semana, mc_pct, faturamento}`, janela fixa de 42 dias terminando no fim do período selecionado)/`category_id`. `resumo.causa_variacao` = decomposição EXATA da variação de margem vs. período anterior (`componentes[]` de faturamento/custo/imposto/tarifa/frete comprador/frete vendedor, soma bate com `delta_margem`) + `mix` (% do faturamento em SKUs com MC abaixo do limiar, atual vs. anterior). **`saude`** = `{score, subscores:{rentabilidade,crescimento,eficiencia_logistica,saude_estoque,concentracao_portfolio,eficiencia_comercial}}`, cada um 0-100, score = média simples dos 6. **`mudancas`** = 8 destaques de maior alta/queda por SKU vs. período anterior (`maior_aumento_faturamento`, `maior_queda_faturamento`, `maior_aumento_margem`, `maior_queda_margem`, `maior_aumento_frete`, `maior_aumento_tarifa`, `maior_aumento_pedidos`, `maior_queda_pedidos` — cada um `{item_id,title,valor_atual,valor_anterior,delta}` ou `null` se nada mudou). `acoes` é o ranking Top 10 por impacto estimado, cada uma com problema/evidência/ação/esforço/confiança/**premissa explícita**; ações `REPRECIFICAR` trazem `cenarios[]` (+3%/+5%/+10%, cada um com impacto estimado próprio). Limiares e fórmulas completos em `business-rules.md`. Módulo `bi` (só admin) |
| `POST /api/bi/margem/narrativa` `{days\|(date_from&date_to), store_id, category_id}` | **Inteligência de Margem — Fase 2 (IA sob demanda)**: reusa `computarMargem()` (mesmos parâmetros de período/filtro do GET acima) e manda um contexto REDUZIDO (resumo + saúde + mudanças + produtos críticos/estrela/ruptura + top ações, nunca o portfólio inteiro) pro Haiku (`server/src/ai/margemNarrativa.js`), que só INTERPRETA em texto — nunca recalcula margem. Devolve `{ok, narrativa:{resumo_executivo, causa_raiz, o_que_eu_faria_agora[]}, periodo, gerado_em, cache:'hit'\|'miss'}`. 503 se `ANTHROPIC_API_KEY` não configurada (`{error}`). **v87: cacheada 30min** por `(days,store_id,category_id,date_from,date_to)` — mesmo filtro dentro do TTL devolve a resposta idêntica sem chamar o LLM de novo (`server/src/db/cached.js`, ver `redis.md`). Módulo `bi` (só admin) |
| `GET /api/bi/margem/produto/:itemId?days&store_id` | **Inteligência de Margem — Fase B (drill-down de 1 produto)**: consulta DEDICADA (não reusa `computarMargem`, que processa o portfólio inteiro) — filtra `VENDA_DETALHE_SELECT` por `o.item_id`, agrega por DIA (não semana). Devolve `{item_id, dias, serie[], resumo}`. `serie[]` = 1 ponto por dia com venda: `{dia, faturamento, margem, mc_pct, preco_medio, frete_vendedor, tarifa, custo, qtd, pedidos}` — alimenta o gráfico temporal do modal em `bi-margem-produtos.html`. `resumo` = soma do período inteiro (mesmos campos de `produtos[]` de `/margem`) + `unit_price_atual`/`imposto_pct` (necessários pro simulador de preço, calculado 100% no cliente). SKU sem venda na janela devolve `serie:[]`/`resumo:null` (não é erro). Módulo `bi` (só admin) |
| `GET /api/bi/rankeamento?days\|(date_from&date_to)&store_id` | **Vendas por Estágio** — cruza o estágio ATUAL de rankeamento (`ranking.buscarFasePorItemIds`, nunca duplicado — fonte é `ranking_ads`/`ranking_ad_links`) com as vendas/margem do período (mesma fórmula de `computarMargem`). Devolve `{periodo, visao_executiva:{estagio_lider_hoje,score[]}, por_fase_periodo[], resumo_hoje, vendas_hoje[], comparacao_ontem[], comparacao_7d[], serie_diaria[], produtos_por_estagio[], recuperacao_antes_depois[], insights[]}`. Buckets sempre incluem os 4 estágios reais (`rankeando`/`ranqueado`/`monitoramento`/`recuperacao`) + `sem_rankeamento` (anúncio nunca marcado no módulo — não descartado). `conversao_pct` usa `item_visits` real, `null` (nunca `0` forçado) quando não há visita cadastrada pro bucket. Estágio é sempre o ATUAL do anúncio, nunca uma reconstrução histórica (`ranking_ads` não guarda histórico confiável de mudança de fase — ver `business-rules.md`). `recuperacao_antes_depois` usa só o carimbo `recuperacao_started_at` (único dado confiável), vendas/dia 14 dias antes vs. desde que entrou — efeito OBSERVADO, nunca causal. Período: `days` (padrão) **ou** `date_from`/`date_to` explícitos (Hoje/personalizado — mesmo contrato de `/bi/margem`); "Hoje" na prática é só `days=1`, não precisa de `date_from`/`date_to`. As janelas "hoje"/"ontem"/"7 dias" das comparações ficam sempre ancoradas no dia real, independente do período personalizado escolhido. Módulo `bi` (só admin) |
| `GET /api/bi/rankeamento/relatorio?days\|(date_from&date_to)&store_id` | **v88 — Relatório em PDF** (botão "Gerar Relatório", `bi-rankeamento.html`): reusa `computarRankeamento()` por inteiro pro período ATUAL (mesmo filtro da tela) e soma 2 janelas comparativas via `porFaseNoPeriodo()` (agregação por fase isolada, sem hoje/ontem/7d/produtos/recuperação/score — nunca uma 2ª fórmula de margem/agregação): o período **anterior** imediato (mesma duração, termina 1 dia antes do atual) e a **mesma janela um mês atrás** (`de`/`ate` −1 mês, `addMesesStr`). Devolve `{periodo_atual, periodo_anterior, periodo_mes_passado, visao_executiva, por_fase:{atual,anterior,mes_passado}, produtos_por_estagio, insights}` — `produtos_por_estagio` é sempre do período ATUAL (o relatório detalha "o que vendeu agora", os outros 2 períodos só entram no comparativo agregado por estágio). Frontend monta o PDF via janela de impressão (`window.print()`, mesmo padrão de `financeiro-dre.html`/`financeiro-compras.html`, sem lib nova). Módulo `bi` (só admin) |
...
| `GET /api/config/frete-motoboy` | **v85**: `{ frete_motoboy_ativo, frete_motoboy_valor }` — flag+valor globais (não por loja/pedido), `app_config.key IN ('frete_motoboy_ativo','frete_motoboy_valor')`, padrão `{false, 12.90}` (sem registro salvo). Controle em `bi-vendas.html` |
| `PATCH /api/config/frete-motoboy { frete_motoboy_ativo?, frete_motoboy_valor? }` | grava flag/valor (`frete_motoboy_valor` precisa ser número ≥0, senão 400). Lida por `calcularMargemLinha` (`vendaMargem.js`) — quando ativa, substitui `frete_vendedor` por `MAX(0, valor − shipping_seller_reembolso)` só nas vendas Flex (`shipping_type='self_service'`). Afeta os mesmos pontos de `imposto_flex_ativo` (`/vendas/detalhado`, `/bi/margem*`, `/pedidos/:id/detalhes`, `/vendas/margem`). Ver `business-rules.md` pra fórmula completa |
...
| `GET /api/embalagem/relatorio-periodo?from&to&store_id&marketplace` | **Aba "Relatório do Dia"** — payload ÚNICO do fechamento de um período (dia, semana seg-sex ou mês). `from`/`to` obrigatórios em `YYYY-MM-DD` (400 se ausentes, malformados ou invertidos); recorte sempre por data local `America/Sao_Paulo`. Devolve `periodo{from,to,dias_no_periodo}`, `resumo{bipagens, pedidos, tempo_total, pedidos_com_tempo, tempo_medio_pedido (SUM/SUM, `null` sem gravação cronometrada), dias_com_movimento, embaladores, media_por_dia, melhor_dia, hora_pico, erros}`, `por_dia` (zero-fill), `por_hora` (0-23 zero-fill), `por_embalador`, `por_loja`, `por_marketplace`, `por_logistica` (rotulada), `top_produtos` (top 30 por título, com `pedidos`/`unidades`) e `erros{por_tipo, recentes}`. São 10 queries pequenas em paralelo, não uma gigante — as rotas anteriores não serviam (`/videos` tem LIMIT 100, `/historico` só a série diária, `/relatorio` só por embalador, e nada agregava PRODUTO embalado). Ver `embalagem.md` |
...
| `GET /api/embalagem/auditoria?only_printed&only_missing&days&date_from&date_to&search` | **Aba Auditoria — "expedição do dia"**: filtro de período pela **data da venda** (`date_created` em SP) — `days` (últimos N, ex. 3/5/10/15/21/30) OU `date_from`/`date_to` (personalizado); sem nada = tudo. **`search`** (nº do pedido / `shipping_id` / rastreio, `ILIKE`) faz busca direta e **ignora status/logística/período** — pra checar se um pedido específico foi ou não bipado, mesmo já expedido. pedidos que precisam sair (ML FLEX/Mercado Envios com `shipping_status='ready_to_ship'`; Shopee Entrega Rápida/Agência com `order_status` READY_TO_SHIP/PROCESSED) cruzados com `packing_videos` → `{items:[{order_id, title, store_nickname, marketplace, logistica, printed, bipado, bipado_at, date_created, etiqueta_impressa_at}], resumo:{total, bipados, faltando, por_logistica}}` (`bipado_at` = quando o vídeo foi gravado, LATERAL no `packing_videos`). Base no **status de envio**, não na data (decisão do usuário — corte por horário descartado). `etiqueta_impressa_at` vem de `print_jobs.printed_at` (data/hora real da impressão). Filtros `only_missing`/`only_printed`. Ver `embalagem.md` |
...
Extensão (público, `routes/extensionAnalise.js`, montado em `/extension` antes do gate): `GET /extension/produto-ativo`, `POST /extension/anuncio` (coleta manual → produto ativo), `GET /extension/monitor/:mlb` (histórico de preço do MLB pro mini-gráfico do painel — `{historico, delta30d, count}`, read-only). **Monitoramento automático (background da extensão)**: `GET /extension/monitoramento/proximos?limit=N&force=1` → `{itens:[{ml_id, url}]}` — concorrentes (`analise_product_ads`, `monitorar=true`, **com `link` não vazio** — sem permalink a aba abriria numa URL inválida/404) mais desatualizados (`last_checked_at` nulo ou > 24h, recoleta 1×/dia), dedup por `ml_id`; `force=1` **ignora o corte de 24h** (recoleta todos agora, limite maior) — usado pelo botão "Forçar recoleta de TODOS agora" do popup; `POST /extension/monitoramento` (body `{rawData}`) → casa pelo `ml_id`, atualiza o anúncio em **todos** os produtos onde é monitorado, carimba `last_checked_at` e alimenta `analise_monitor_snapshots` — **não** depende do produto ativo. Ver `analise-produtos.md`.
