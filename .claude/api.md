...
| `GET /api/bi/margem?days&store_id` | **Inteligência de Margem** (BI) — motor determinístico, sem LLM: agrega `/vendas/detalhado` por `item_id` e devolve `{periodo, resumo, produtos[], bandas_margem[], concentracao, frete_tarifa:{por_logistica,por_conta}, acoes[]}`. Cada produto traz faturamento/qtd/margem/MC%/frete%/tarifa%/classificação (ESTRELA/VOLUME_BAIXA_MARGEM/ALTA_MARGEM_BAIXO_VOLUME/DESTRUIDOR_MARGEM/PREJUIZO/NEUTRO)/estoque+ruptura (mesma fórmula de `getRupturaEstoque`)/score 0-100 com breakdown. `acoes` é o ranking Top 10 por impacto estimado, cada uma com problema/evidência/ação/esforço/confiança/**premissa explícita**. Limiares e fórmulas completos em `business-rules.md`. Módulo `bi` (só admin) |
