...
| `GET /api/conciliacao/extrato?store_id&date_from&date_to&tipo&q&page&limit` | extrato da conta MP (`mp_account_movements`, fase 2) — cada crédito/débito paginado. `tipo` filtra por `description` (Payment/Cash withdrawal/Refund/...). Exclui linhas `record_type='Total'`. `{movimentos, total, page, limit}` |
| `GET /api/conciliacao/saques?store_id` | lista os movimentos `description='Cash withdrawal'` (transferências pro banco) — `{saques: [{release_date, valor, balance, store_nickname}]}` |
