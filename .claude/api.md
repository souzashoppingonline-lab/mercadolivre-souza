...
| `GET /api/vendas/detalhado?store_id&status&days&search&date_from&date_to` | linha a linha com margem calculada por pedido — fórmula em `finance.md`. **Tarifa e frete do vendedor por pedido vêm da Conciliação Bancária** (`mp_account_movements`, `description='Payment'`, `LEFT JOIN LATERAL` por `order_id`, `ABS()`): `tarifa=mp_fee_amount`, `frete_vendedor=shipping_fee_amount`; fallback `orders.ml_fee`/`shipping_seller_cost` quando não há relatório. Cada linha traz `tem_conciliacao`; `summary` traz `pedidos_conciliados` (cobertura) |
...
| `GET /api/vendas/margem?days` ou `?date_from&date_to` (opc. `frete_comprador=1`) | **Margem de Contribuição por loja** (mesma fórmula do Mercado Turbo): `Aprovadas − Custo − Imposto − Tarifa − Frete Vendedor`. Devolve `{lojas:[{store_id,loja,faturamento,canceladas,aprovadas,qtd_aprovadas,custo,imposto,tarifa,frete_vendedor,frete_comprador,pedidos_conciliados,tem_conciliacao,margem,margem_pct}], considerar_frete_comprador}`. **Tarifa e frete do vendedor vêm da Conciliação Bancária** (`mp_account_movements`, `description='Payment'`, casado por `order_id`, `ABS()` no valor): `tarifa=mp_fee_amount`, `frete_vendedor=shipping_fee_amount`. Sem relatório MP no período, `tarifa` cai para `orders.ml_fee` do pedido (`tem_conciliacao=false`) e `frete_vendedor=0`. `custo=items.cost×qtd`, `imposto=total_amount×stores.imposto_pct/100`. Frete do comprador só é abatido com `frete_comprador=1` (toggle, igual ao Turbo). Só pedidos não cancelados |
