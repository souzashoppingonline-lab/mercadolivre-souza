...
| `GET /api/shopee/chat?store_id` | conversas Shopee com mensagem não lida (`shopee_chat`, `unread_count>0`) — `{rows:[{conversation_id,buyer_name,unread_count,last_message,last_message_type,last_message_ms,conta}], resumo:{conversas,nao_lidas}}`. Preenchido pelo job `syncShopeeChat` (worker). Responder é pelo app da Shopee |
