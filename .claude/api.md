...
| `PATCH /api/ranking/ads/:id` `{active?, milestone_every?, fase?, ads_investido?, roas?, orcamento_diario?, preco_anterior?, preco_atual?}` | pausar/retomar, mudar intervalo do marco, mudar de **fase** (`rankeando`↔`ranqueado`; `ranqueado` carimba `ranqueado_em`), ou salvar as métricas **manuais** do ciclo (ADS/ROAS/orçamento + preço anterior/atual; `''`/null zera a coluna) |
