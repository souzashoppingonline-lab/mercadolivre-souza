...
Extensão (público, `routes/extensionAnalise.js`, montado em `/extension` antes do gate): `GET /extension/produto-ativo`, `POST /extension/anuncio` (coleta manual → produto ativo), `GET /extension/monitor/:mlb` (histórico de preço do MLB pro mini-gráfico do painel — `{historico, delta30d, count}`, read-only). **Monitoramento automático (background da extensão)**: `GET /extension/monitoramento/proximos?limit=N` → `{itens:[{ml_id, url}]}` — concorrentes (`analise_product_ads`, `monitorar=true`) mais desatualizados (`last_checked_at` nulo ou > 24h, recoleta 1×/dia), dedup por `ml_id`; `POST /extension/monitoramento` (body `{rawData}`) → casa pelo `ml_id`, atualiza o anúncio em **todos** os produtos onde é monitorado, carimba `last_checked_at` e alimenta `analise_monitor_snapshots` — **não** depende do produto ativo. Ver `analise-produtos.md`.
