...
- `GET /politica-de-privacidade` e `GET /politica-seguranca` → páginas públicas autocontidas (`politica-de-privacidade.html`/`politica-seguranca.html` na raiz do repo), exigidas pelo cadastro de app no TikTok Shop Partner Center (e formulários semelhantes de outros marketplaces). Rota explícita em `server.js` (fora do SPA fallback) e whitelisted em `staffAuth.js` `PUBLIC_PREFIXES` — abrem sem login mesmo com `STAFF_AUTH_ENABLED=true`, pra um revisor externo conseguir ler. Conteúdo tem 2 campos com placeholder (`[PREENCHER: ...]`) — razão social/CNPJ, e-mail de contato e a seção de infraestrutura de rede — preencher antes de submeter ao TikTok. Ver `.claude/tiktok.md`.
