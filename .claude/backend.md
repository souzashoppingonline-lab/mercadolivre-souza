...
- `GET /politica-de-privacidade`, `GET /politica-seguranca` e `GET /protecao_dados_pessoais` → páginas públicas autocontidas (`politica-de-privacidade.html`/`politica-seguranca.html`/`protecao_dados_pessoais.html` na raiz do repo), exigidas pelo cadastro de app no TikTok Shop Partner Center (e formulários semelhantes de outros marketplaces). Rota explícita em `server.js` (fora do SPA fallback) e whitelisted em `staffAuth.js` `PUBLIC_PREFIXES` — abrem sem login mesmo com `STAFF_AUTH_ENABLED=true`, pra um revisor externo conseguir ler. `protecao_dados_pessoais.html` é a política INTERNA (como a equipe trata dado pessoal no dia a dia — classificação, acesso, retenção, resposta a incidente), diferente das outras duas (privacidade = pro titular externo; segurança = controles de infraestrutura). Todas têm campos com placeholder (`[PREENCHER: ...]`) — razão social/CNPJ, e-mail de contato, infraestrutura de rede, treinamento da equipe — preencher antes de submeter ao TikTok. Ver `.claude/tiktok.md`.
