...
Escopo: itens com `item_seo_score.catalog_listing = true` **ou** alocados no estágio Catálogo do Rankeamento (`ranking_ads.fase='catalogo'`, v88 — ver `rankeamento.md`). "Ganhando" é decidido comparando `catalog_competition.winner_item_id` ao próprio `item_id` — **não** pela string `catalog_competition.status` (só o valor `"competing"` foi confirmado ao vivo contra a API real; outros valores possíveis não foram documentados, então a UI/summary do módulo Qualidade de Anúncio nunca dependem do texto exato). `summary.perdendo_buybox` (rota `GET /api/qualidade-anuncio`) conta itens `catalog_listing=true` com `winner_item_id` preenchido e diferente do próprio item — itens ainda sem sync (`winner_item_id IS NULL`) não entram nem como "ganhando" nem como "perdendo".

**Exceção (v88, card do estágio Catálogo apenas):** o usuário forneceu, nesta tarefa, o enum oficial documentado pela API (`status`: `winning`/`competing`/`sharing_first_place`/`listed`) — informação que o projeto não tinha confirmado ao vivo antes. Com base nisso, o card do estágio Catálogo (só ali, não no módulo Qualidade de Anúncio) também confia em `buybox_status === 'sharing_first_place'` pra distinguir "compartilhando o 1º lugar" de "perdendo" — a ÚNICA string além da comparação por `winner_item_id` em que se confia. Qualquer outro valor de status continua tratado como texto informativo, nunca como base de decisão.
...
- **Alerta por SKU/comprador na hora do bipe (v100 — Embalagem)**: mesma fórmula de taxa acima, só que por `item_id` (não por loja/dia) e numa janela de 90 dias (`DEVOLUCAO_JANELA_DIAS`, `routes/embalagem.js`), com amostra mínima **5 pedidos** (`DEVOLUCAO_SKU_AMOSTRA_MIN`) e limiar **10%** (`DEVOLUCAO_SKU_ALERTA_PCT`) — mais alto que o 5% do alerta diário porque é um recorte mais granular (por item, não por loja inteira), naturalmente mais ruidoso. Cobre ML (`returns`) e Shopee (`shopee_returns`, tabela separada). Alerta irmão por comprador: 2+ devoluções do mesmo `buyer_nickname` na mesma janela (`DEVOLUCAO_COMPRADOR_ALERTA_MIN`) — **só ML**, Shopee não libera o comprador pro app. Detalhes de onde aparece em `embalagem.md`.
...
- **Causa**: `tarifa_real = ABS(SUM(m.mp_fee_amount))` (`mp_account_movements`, `description='Payment'`) soma TODAS as linhas "Payment" da Conciliação pra aquele `order_id`. Quando o pedido tem frete pago pelo comprador (não é frete grátis completo), o relatório de liberação do Mercado Pago às vezes gera uma 2ª linha "Payment" pro repasse desse frete, com o valor caindo inteiro na coluna `MP_FEE_AMOUNT` (nada em `SHIPPING_FEE_AMOUNT`) — a soma cega junta comissão real + frete do comprador.
- **Correção**: dentro da própria LATERAL, quando (1) há **mais de 1** linha Payment pro pedido, (2) o `ABS(mp_fee_amount)` de uma delas bate **exatamente** com `orders.shipping_cost` (frete pago pelo comprador) e (3) essa linha não tem `shipping_fee_amount` próprio — essa linha é excluída da soma de tarifa. Pedido de 1 linha só (o caso comum) nunca é tocado, mesmo que por coincidência algum valor bata com o shipping_cost — evita zerar uma comissão real por coincidência numérica.
- **Validado contra um Postgres real** (não só mock de `pool.query`, que não pega bug de sintaxe/semântica SQL — foi assim que a 1ª versão da proteção passou despercebida com um bug de SINAL: `mp_fee_amount = shipping_cost` nunca batia porque `mp_fee_amount` vem negativo do relatório, precisa de `ABS()`). 4 cenários testados num Postgres 16 local com dados sintéticos: pedido com o bug (2 linhas, frete vazado) → corrige; pedido normal (1 linha) → não toca; 1 linha que bate por coincidência → não toca (exige >1 linha); 2 linhas legítimas sem coincidência (ex. split de pagamento) → soma normal.
...
- **Maior precedência de todas as fontes**: `tarifa_manual` vence Conciliação/`ml_payments`/`ml_fee` em TODOS os 4 lugares que calculam tarifa (`calcularMargemLinha`/`VENDA_DETALHE_SELECT`, o card de totais de `/vendas/detalhado`, `/pedidos/:id/detalhes`, `getMargemPorLoja`) — mesmo raciocínio de não deixar telas diferentes mostrarem tarifa diferente pra mesma venda (ver seção acima). `fonte_taxa`/`fonte` ganha o valor `'manual'` nos 3 lugares que já expõem essa informação.
- **Por PEDIDO, não por anúncio** — diferente do custo (`items.cost`, edita todos os cards daquele MLB de uma vez), a tarifa é específica de cada venda (frete/logística/promoção podem variar pedido a pedido), então editar um card nunca afeta outro.
- **`tarifa_manual=NULL` (padrão)** = sem edição, usa o cálculo automático de sempre (Conciliação → `ml_payments` → `ml_fee`). Botão "desfazer" (só aparece quando `fonte_taxa==='manual'`) manda `tarifa: null` explícito pro backend, que limpa `tarifa_manual`/`tarifa_manual_by`/`tarifa_manual_at` e volta pro cálculo automático — nunca precisa adivinhar o valor "de antes".
...
- **Explicação "Como cada score funciona"** (`bi-margem.html`, fim da página, pedido explícito do usuário): card com os 6 sub-scores listados do **pior pro melhor** (o que mais precisa de atenção primeiro), cada um com "o que mede" + faixas do score + uma dica de melhoria. As dicas são **sempre genéricas/de playbook** (ex.: "veja a aba Estoque & Ruptura", "reduza o custo do produto") — **nunca** uma causa específica inventada pra aquele período (mesmo princípio de nunca afirmar causa sem evidência já usado em toda a Inteligência de Margem); pra crescimento, a dica aponta pro card "Por que a margem mudou" (decomposição exata), em vez de adivinhar o motivo. Sub-score já em 100 mostra "✅ já está no máximo" no lugar da dica. 100% client-side (`SAUDE_SUB_EXPLICACAO`, `bi-margem.html`) — sem endpoint novo, só descreve em português o que `calcularSaude()` já calcula.
...
- **`sem_rankeamento`** é um bucket próprio (não um erro nem um item descartado) — a maioria dos anúncios do catálogo nunca foi marcada no módulo Rankeamento (ele é opt-in). Fica de fora dos "4 blocos de estágio" (não é um estágio de processo) mas aparece na tabela de comparação completa, pra os totais não mentirem.
...
- **Filtro de período: `days` (7/14/30/60/90), "Hoje" ou personalizado** (`date_from`/`date_to`) — pedido explícito do usuário. "Hoje" **não precisa** de `date_from`/`date_to`: resolve sozinho com `days=1` (`periodoIni = hoje − 0 dias = hoje`), então não é tratado como um caso especial no backend. Personalizado segue o MESMO contrato de `computarMargem`/Fase A (`date_from`/`date_to` explícitos vencem `days`, duração real calculada do intervalo). **As janelas "hoje"/"ontem"/"7 dias fechados"** (usadas nas comparações `comparacao_ontem`/`comparacao_7d`/`resumo_hoje`) ficam **sempre ancoradas no dia real atual**, independente do período principal escolhido — navegar pra um período personalizado no passado não muda o que "hoje" significa nessas comparações (são sobre "como está indo agora", não sobre o fim do intervalo navegado).
- **Relatório em PDF (v88, `GET /api/bi/rankeamento/relatorio`)**: pedido explícito do usuário — comparar o período do filtro selecionado com (1) o período **imediatamente anterior**, de mesma duração, e (2) a **mesma janela um mês atrás** (mesmos dias de início/fim, −1 mês — não "o mês calendário anterior inteiro"). Ex.: filtro "últimos 7 dias" (24-30/ago) compara com 17-23/ago (anterior) e 24-30/jul (mês passado). O período anterior é derivado da duração REAL do período atual (`periodo_atual.dias`), então funciona igual pra `days=N` e pra intervalo personalizado. A tabela de vendas por anúncio dentro de cada estágio é sempre do período ATUAL — comparar produto a produto entre 3 janelas diferentes não foi pedido e misturaria estoque/preço de datas diferentes sem contexto.

## Ações Recomendadas — feedback loop OBSERVADO, nunca causal (`business_insights.concluida_em`, v87)

> Pedido explícito do usuário. A tabela `business_insights` (v83) já persistia status/nota manuais mas ficou deliberadamente SEM feedback loop 3 vezes seguidas (Fase 2, Fase A, Fase F — ver `decisions.md`) pelo mesmo motivo: medir resultado real e atribuí-lo à ação afirmaria causalidade sobre uma variação que pode ter dezenas de outras causas (sazonalidade, concorrência, mudança de custo) — proibido pela própria especificação original. A saída (já usada em `rankeamento.md` pro antes×depois de Recuperação): mostrar o efeito OBSERVADO, com a linguagem deixando claro que não é prova de causa.

- **`concluida_em`** (nova coluna) é só um CARIMBO de quando a ação entrou em `status='concluida'` — nunca um valor calculado/congelado. Some (`NULL`) sempre que o status sai de `'concluida'` de novo — reabrir e concluir depois começa uma janela de medição NOVA, não reaproveita a antiga. Reenviar `status='concluida'` enquanto já está concluída (ex.: só editando a nota) **não reseta** o carimbo — só a transição PRA `'concluida'` grava `now()` (`CASE WHEN EXCLUDED.status='concluida' AND business_insights.status IS DISTINCT FROM 'concluida' THEN now() ...` no upsert de `PATCH /margem/acoes-status`).
- **`GET /api/bi/margem/acoes-feedback`** (1 request em lote pra tela toda, não N por ação) calcula, pra cada ação `concluida` com carimbo: "antes" = 14 dias fechados imediatamente ANTES de `concluida_em`; "depois" = desde `concluida_em` até agora. **Mesma janela/taxa-por-dia de Recuperação** (`vendas_dia_antes`/`vendas_dia_depois` em `rankeamento.md`) — antes sempre ÷14, depois ÷dias reais decorridos (mínimo 1) — janelas de tamanho diferente, comparar taxa (não total bruto) é o que torna os dois lados comparáveis. Reusa `VENDA_DETALHE_SELECT`/`calcularMargemLinha` (nunca uma 2ª fórmula de margem) — nada é recalculado/persistido como "verdade" além do carimbo em si.
- **UI** (`bi-margem-acoes.html`): bloco "Efeito observado desde a conclusão (data, há N dias)" aparece só em cards com `status='concluida'` **e** dado de feedback disponível (ação concluída há pouco tempo, sem pedido "depois" ainda, simplesmente não mostra o bloco — nunca inventa número). 3 métricas: Faturamento/dia e Margem/dia (variação relativa %, seta verde/vermelha), MC% (diferença em **pontos percentuais**, não variação relativa — 20%→30% é "+10pp", nunca "+50%", que seria a leitura errada de "quanto a % mudou em %"). Texto obrigatório "NÃO é causalidade garantida — outras coisas também mudam nesse intervalo", mesma disciplina de linguagem do antes×depois de Recuperação.

## Análise da IA — cache de 30min por filtro (`POST /api/bi/margem/narrativa`, v87)

> Pedido explícito do usuário: "hoje todo clique chama o LLM de novo mesmo sem nada ter mudado no período" — a narrativa é sobre padrão do PERÍODO (causa raiz, resumo executivo), não sensível ao segundo mais recente.

- **Chave de cache** = `days`+`store_id`+`category_id`+`date_from`+`date_to` (os mesmos parâmetros de `parseMargemParams`) — clicar de novo com o MESMO filtro dentro do TTL devolve a resposta idêntica (inclusive o `gerado_em` original) sem chamar o LLM de novo. Filtro diferente é sempre um cache MISS novo.
- **TTL 30min, só por tempo — sem invalidação por evento**, mesmo padrão já usado em `/vendas/detalhado`/`/vendas/margem` (ver `redis.md`/`decisions.md`): invalidar em todo `order_updated` seria frequente demais pro benefício, e a narrativa não precisa de frescor ao segundo.
- **Erro do LLM nunca fica em cache** — o helper `cached()` (`server/src/db/cached.js`, extraído de `routes/api.js` quando `routes/bi.js` virou o 2º consumidor) só grava no Redis depois que a função passada roda com sucesso; uma exceção propaga sem nunca chamar `redis.set`.
- **Resposta traz `cache: 'hit'|'miss'` e `gerado_em`** — a UI (`bi-margem.html`) mostra uma linha discreta "Resultado em cache — gerada há N min" ou "Análise gerada agora", só transparência visual, o botão funciona igual dos dois jeitos.
