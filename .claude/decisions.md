...
**Outro efeito colateral encontrado**: pedidos de teste (sandbox/mock) gravados em `orders` durante o desenvolvimento contaminaram silenciosamente KPIs agregados do ML (`/dashboard/kpis` soma `orders` de todas as lojas sem filtrar marketplace) — um pedido mock com `date_created` de hoje chegou a somar no card "vendas hoje" real. **Ação tomada**: os pedidos de teste foram apagados (`DELETE ... WHERE store_id = 9000000001`). **Risco em aberto, registrado em `todo.md`**: `routes/api.js` tem 30+ pontos que leem `orders` sem filtrar `marketplace_id` — qualquer pedido Amazon real (produção) vai aparecer misturado nos KPIs/relatórios do ML até essas queries serem revisadas deliberadamente (decisão de produto pendente: unificar tudo por padrão, ou manter telas ML-only por padrão com Amazon opt-in).
