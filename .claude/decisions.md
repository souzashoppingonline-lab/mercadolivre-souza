...
**Efeito colateral encontrado em produção e corrigido na mesma tarefa**: a store sentinela da Amazon (`id=9000000001`) vive na mesma tabela `stores` usada pelo OAuth do ML. Quase toda função de `worker.js` que itera lojas (`startWorkers`, `tokenRefreshLoop`, `syncVendas`, `syncMetricas`, `syncParentItems`, `syncReturns`, `syncVisitas`, `syncScores`, comandos `/status`/`/refresh` do bot) faz `SELECT ... FROM stores` sem filtro, assumindo que toda linha é uma loja ML real. Sem exclusão explícita, a sentinela era tratada como loja ML com token "epoch zero", chegando a **disparar alerta falso no Telegram** ("Loja Amazon desconectada... reconecte") a cada 30 min via `tokenRefreshLoop`, além de ganhar uma fila `ml-webhooks-9000000001` ociosa em `startWorkers`. **Correção**: todas essas queries passaram a excluir `WHERE id <> 9000000001`. **Lição para o futuro**: se um segundo marketplace de conta única (Shopee, Magalu) precisar da mesma store sentinela, cada uma dessas queries precisa da mesma exclusão — ou, melhor ainda, trocar o padrão por `WHERE marketplace_id = (SELECT id FROM marketplaces WHERE code='ML')` num ponto único reutilizável, em vez de excluir IDs individualmente.
