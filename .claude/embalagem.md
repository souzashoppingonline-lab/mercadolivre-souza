...
- **Diagnóstico manual** (`server/scripts/verificarEtiquetaShopee.js`): script standalone pra investigar "etiqueta bipada não encontrada" fora da rota, com log de cada etapa (match exato/parecido do tracking, quantos pedidos estão sem tracking dentro/fora da janela de 20 dias, teste ao vivo dos candidatos contra a API oficial da Shopee, e — opcional, 2º argumento — busca direta pelo `order_sn` impresso na etiqueta física em "Pedido:", que não depende do tracking estar preenchido). Uso: `node server/scripts/verificarEtiquetaShopee.js <tracking bipado> [order_sn]`.
- **Causa recorrente identificada (08–12/09/2026) e CORRIGIDA**: o pedido nem existia ainda em `orders`/`shopee_order_data` — etiqueta gerada pela Shopee quase no mesmo instante do bipe, antes do webhook/polling sincronizar. Aconteceu 3x seguidas em produção (não era caso isolado), sempre com o mesmo padrão (pedido do dia, `order_sn` com prefixo da data de hoje). `refreshShopeeTrackingOnDemand` **não resolvia** esse caso porque só itera pedidos que JÁ têm linha local (`shopee_order_data.tracking_number IS NULL`) — um pedido inexistente localmente nunca entra nesse universo. **2º nível de fallback adicionado**: `liveLookupNewShopeeOrder(tracking)` — busca a lista de pedidos recentes (últimas 6h) AO VIVO na API da Shopee (`listRecentOrders`, mesma chamada do polling), sem depender de já existir localmente; testa o rastreio de cada um e, ao achar o bipado, grava um upsert mínimo em `orders`/`shopee_order_data` (`ml_id`/`order_sn`, `status` via `mapShopeeStatus` — reaproveitada de `marketplaceEventWorker.js`, exportada pra não duplicar a regra —, `raw_data`, `tracking_number`) pra `lookupShopeeByTracking()` achar na sequência. O sync completo (escrow/logistics/Telegram/ranking) continua vindo depois, no próximo ciclo normal do worker — este fallback só desbloqueia o bipe na hora. Chamado só quando `refreshShopeeTrackingOnDemand` já falhou (2º nível, mesma exceção de arquitetura já documentada acima: rota chamando client de marketplace por ação pontual de bipagem).
