...
**Leitor USB com layout de teclado incompatível — caracteres do JSON saem trocados**: um leitor 2D USB (emula teclado HID) testado pelo usuário lê o mesmo QR FLEX só que com os caracteres especiais do JSON substituídos por causa de um layout de teclado diferente do esperado pelo SO: `"` sai como `^`, `:` sai como `ç`, o `{` de abertura some e o `}` de fechamento sai como `{`. Exemplo real capturado: `^id^ç^47538810049^,^sender_id^ç1662123376,...,^security_digit^ç^0^{` (equivale a `{"id":"47538810049","sender_id":1662123376,...,"security_digit":"0"}`). Sem tratamento, isso quebrava o `JSON.parse` e caía no fallback de "só dígitos", que concatenava **todos** os números da string inteira (id + sender_id + dígitos do hash_code) em vez do `shipping_id` real. `parseShippingId()` agora tenta um reparo (`repairScannerGarbling()`: desfaz as trocas, repõe o `{`/`}`) antes de cair no fallback antigo — não muda o comportamento do celular/leitores que já produzem JSON válido.
