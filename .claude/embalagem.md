...
Pedido do usuário. O campo "Estoque atual" já existia na grade de detalhes do card de bipagem (`items.available_quantity`, dado já carregado — nenhuma query nova), só não chamava atenção. Quando `available_quantity < 20` (`ESTOQUE_BAIXO_EMBALAGEM`, módulo `pages/embalagem.html`) o número fica vermelho piscando (`embEstoqueBlink`, cópia de `embPesoBlink` recolorida — mesmo racional do peso na balança, v101) + uma linha de aviso: "⚠️ Estoque baixo — verifique antes de finalizar."
