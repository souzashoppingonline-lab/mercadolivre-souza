...
2. **`ml_payments`** (webhook `payments`, tempo real, por venda) — `taxa = transaction_amount − net_received_amount` (só `status='approved'`, `net_received_amount` não nulo). É o **total** ML descontado (comissão + frete + taxas juntos). Como o ML não separa comissão de frete nesse número, a rota **separa usando o `shipping_seller_cost`** já conhecido (do `handleShipment`/edição manual): **Frete Vendedor = LEAST(shipping_seller_cost, taxa)** e **Tarifa = taxa − esse frete** (o `LEAST` impede tarifa negativa). A soma tarifa+frete continua igual à `taxa`, então **a Margem/MC% fica idêntica** — muda só o rótulo, e a coluna Frete Vendedor deixa de aparecer zerada na tabela (bate com o modal, que mostra `shipping_seller_cost`). Se `shipping_seller_cost=0` (frete ainda não sincronizado), cai no comportamento antigo: tudo em Tarifa, frete 0. Cobre a venda nova antes do relatório das 05:40 chegar.
...
Cada linha do `/detalhado` traz `fonte_taxa` (`conciliacao`|`pagamento`|`pedido`) e `tem_conciliacao`; os totais/lojas trazem `pedidos_conciliados` (pedidos com taxa real, fonte 1 **ou** 2). Na fonte `pagamento`, a coluna Frete Vendedor mostra o `shipping_seller_cost` (separado da Tarifa via `LEAST`, item 2 acima) e só fica 0 se o frete ainda não foi sincronizado — a Margem bate igual nos dois casos. Ver `conciliacao-bancaria.md`. As demais rotas da lista acima ainda usam só os campos do pedido.
