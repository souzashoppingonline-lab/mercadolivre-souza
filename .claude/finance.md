...
0. **Edição manual** (`orders.tarifa_manual`, v84) — vence TODAS as fontes abaixo, inclusive quando há Conciliação disponível. Botão ✏️ no card "Resumo por Venda" (`bi-vendas.html`), escape hatch pra quando nenhuma fonte automática acerta. `NULL` (padrão) = sem edição, segue a precedência normal. **O Frete Vendedor tem a MESMA edição manual, em campo separado** (`orders.frete_vendedor_manual`, v86, botão ✏️ próprio) — precedência independente da tarifa (editar um não mexe no outro), mas com a mesma regra de vencer tudo, inclusive a substituição do custo motoboy Flex (abaixo). Ver `business-rules.md`.
