...
- **Início**: monitoramento (v113 — rodízio automático em tela cheia por 12 páginas já existentes do dashboard, 1 minuto cada, reinicia sozinho; ver seção dedicada abaixo), top-vendas-online (dashboard matinal "morning digest" — consolida em cards/gráficos coloridos o que já vai por Telegram/e-mail: top vendas últimas 4h — mesmo dado do alerta Telegram, via `GET /api/schedule/runs?job=top-vendas`, auto-refresh 4h —, resumo do dia anterior, top vendas do dia 24h (com botões CSV/PDF — `dlTopDiaCSV()`/`dlTopDiaPDF()`, 100% client-side, sem rota nova; PDF via jsPDF carregado sob demanda do CDN, mesmo padrão de `vendas-turbo.html` simplificado pra 1 tabela), relatório semanal; usa Chart.js, sem rota exclusiva além de `GET /api/dashboard/resumo-ontem`/`top-vendas-dia`/`resumo-semanal`, ver `api.md`), agenda-trello (quadro Kanban de tarefas do Analista de E-commerce — ver seção dedicada abaixo e `task-engine.md`)
- **Operação**: anuncios, rankeamento (acompanhamento de anúncio novo na janela de ranqueamento — tabela de todos os anúncios + cards "em rankeamento" com contador 5-em-5 e timeline venda-a-venda ao vivo via WS `ranking_event`; ver `rankeamento.md`), pedidos, embalagem (bipagem de etiqueta + vídeo de conferência — ver seção dedicada abaixo e `embalagem.md`), vendas, vendas-por-loja, promocoes (aba "Adicionar Promoções", v108: buscar anúncio por MLB, ver promoções ativas + campanhas do vendedor disponíveis, checar elegibilidade e aderir/sair — sob demanda, ver `api.md`/`mercadolivre.md`), perguntas, mensagens, clientes, lojas
...
- **Módulos** (fora do menu Operacional — sistema próprio via switcher no topbar, ver `modules.md`): financeiro (`pages/financeiro.html`, layout `js/layout-financeiro.js`, "em construção"), inteligencia-negocio (layout `js/layout-bi.js`, `BI_NAV_ITEMS` em 2 seções) — `inteligencia-negocio.html` (Painel Estratégico, "em construção"), `bi-vendas.html` (Resumo por Venda, com tag de estágio de rankeamento por card), as 6 páginas de Inteligência de Margem, cada uma com entrada própria no menu (`bi-margem.html` Visão Geral + `bi-margem-produtos.html` + `bi-margem-portfolio.html` + `bi-margem-frete.html` + `bi-margem-estoque.html` + `bi-margem-acoes.html`, navegação também via barra de abas `js/biMargemTabs.js`, que preserva o filtro na querystring), `bi-rankeamento.html` (Vendas por Estágio — integração com o módulo Rankeamento) e `agente-financeiro.html` (Agente Financeiro — 4 relatórios sob demanda cruzando Postgres operacional + Supabase Financeiro, `routes/agenteFinanceiro.js`) — detalhe completo em `modules.md`
- **Sistema**: mcp (chat com IA), monitor (métricas de servidor + config Telegram), schedule (jobs agendados), webhook (logs de webhooks recebidos), saude-sistema (filas BullMQ + heartbeat dos processos + último webhook/sync, auto-refresh 15s — ver `workers.md`), usuarios (v80: gestão dos usuários de acesso restrito — criar, trocar senha, mudar papel, excluir; **só admin**, some da sidebar dos demais via `adminOnly` em `NAV_ITEMS` — ver `auth-staff.md`)

Fora desse agrupamento (não usam `NAV_ITEMS`/sidebar ML — têm sidebar própria por marketplace, ver `js/layout-amazon.js`/`js/layout-shopee.js`/`js/layout-tiktok.js` abaixo): `dashboard-amazon`, `amazon-vendas`, `amazon-pedidos`, `amazon-produtos`, `amazon-anuncios` (dashboards dedicados da Amazon), `dashboard-shopee`, `shopee-vendas`, `shopee-anuncios`, `shopee-score`, `shopee-precos-estoque`, `shopee-precificador`, `shopee-promocoes`, `shopee-problemas`, `shopee-devolucoes`, `shopee-performance`, `shopee-ia-socio`, `shopee-financeiro`, `shopee-chat`, `shopee-lojas` (dashboards dedicados da Shopee, mesmo padrão — ver `shopee.md`), `dashboard-tiktok`, `tiktok-vendas`, `tiktok-anuncios` (dashboards dedicados do TikTok Shop, mesmo padrão — ver `tiktok.md`).

## Página Monitoramento (`pages/monitoramento.html`, v113) — rodízio de telas em modo TV

Pedido explícito do usuário: uma tela pra deixar numa TV/monitor da sala, alternando sozinha entre as principais páginas do dashboard (venda, performance, horário, dia da semana, anúncio com problema, webhook, venda por loja, embalagem, saúde do sistema, dashboard geral, evolução diária, anúncios), 1 minuto em cada, reiniciando do começo depois da última. **Substitui a antiga tela Expedição como destino do menu principal** — ver decisão abaixo.

- **Mecanismo — reaproveita as páginas existentes via `<iframe>`, não duplica nenhuma lógica de busca/gráfico.** `SLIDES` (array no próprio `monitoramento.html`) lista `{ label, url }`; um `setInterval` de 60s troca `#monIframe.src` pra próxima URL e volta pro índice 0 depois da última — cada slide é uma navegação nova do iframe (não 12 iframes vivos ao mesmo tempo), então o JS/polling/WS de um slide para completamente assim que o rodízio troca de tela.
- **Modo `?embed=tv` (genérico, qualquer página que use `js/layout.js` + `css/style.css` ganha de graça)**: `js/layout.js` checa `new URLSearchParams(location.search).get('embed') === 'tv'` **antes** do `DOMContentLoaded` e adiciona a classe `body.embed-tv`; `css/style.css` esconde `.sidebar`/`.topbar` e zera a margem de `.main-content` só com essa classe, além de aumentar um pouco fontes de elementos compartilhados entre a maioria das páginas (`.kpi-value`, `.data-table`, `.card-header h3`). Central em 2 arquivos — nenhuma das 12 páginas do rodízio precisou de CSS dedicado pra isso, só a Expedição (que já tinha seu próprio Modo TV, ver `embalagem.md`) e a Embalagem (que precisa abrir na aba certa) ganharam um pequeno ajuste, descritos abaixo.
- **Slide "Embalagem"**: abre direto na aba **Conferência do Dia** via `?tab=conferencia` — `embalagem.html` lê esse parâmetro no `DOMContentLoaded` e chama `switchTab('conferencia')` (mesma função usada pelos botões de aba). Nenhuma outra das 12 páginas do rodízio tem abas.
- **Slide "Pedidos" é a própria Expedição** (`expedicao.html?embed=tv`) — ela já tinha seu próprio Modo TV (`body.tv-mode`, botão fullscreen); quando aberta com `embed=tv` ela ativa esse modo sozinha, **sem** pedir fullscreen (quem controla a tela cheia é o Monitoramento, a página de fora) e **removendo** os botões "Modo TV"/"Sair do Modo TV" (não fazem sentido dentro de um slide que troca sozinho).
- **Modo TV do próprio Monitoramento**: mesmo mecanismo de `expedicao.html` (`body.tv-mode`, botão fullscreen, saída pelo Esc nativo) — esconde sidebar/topbar/subtítulo e reposiciona o rótulo "Tela X/12 — nome" como overlay fixo no canto.
- **Sem rota nova, sem tabela nova** — é 100% composição de páginas que já existiam.

**Decisão — Expedição saiu do menu principal, mas continua existindo (pedido explícito do usuário, ver `decisions.md`)**: o time de separação/embalagem já usa a Expedição fixa numa tela própria o dia todo; se ela existisse só dentro do rodízio, esse time veria a lista de pedidos 1 a cada 12 minutos, o que não serve pro uso real. Por isso: `expedicao.html` **saiu de `NAV_ITEMS`** (não é mais destino do menu principal) mas **continua no ar, acessível direto por URL/favorito**, e continua sendo a única página com nav própria pro papel `embalagem` (`EMBALAGEM_EXTRA_PAGES` em `staffAuth.js`, inalterado) — quem precisa dela o tempo todo (o time de separação) não perde nada; quem quer o panorama geral usa o Monitoramento.
...
Mesmo molde exato de `js/layout-amazon.js` (mesma função `document.addEventListener('DOMContentLoaded', ...)`, mesmas classes CSS reaproveitadas, cor de destaque própria — laranja Shopee `#ee4d2d`). `SHOPEE_NAV_ITEMS`: **Dashboard** (`dashboard-shopee.html`), **Vendas Totais** (`shopee-vendas.html`), **Anúncios** (`shopee-anuncios.html`), **Financeiro** (`shopee-financeiro.html`), **Mensagens** (`shopee-chat.html`), **Lojas** (`shopee-lojas.html`). As duas novas usam **Chart.js** (mesma CDN das páginas ML — `cdn.jsdelivr.net/npm/chart.js`) e um **seletor de loja** (`#shpLoja`, populado por `DB.getShopeeLojas()`) pra filtrar por conta ou agregar todas — o backend (`/shopee/vendas`, `/shopee/anuncios`) já aceita `store_id`, então nada é hardcoded a uma loja (multi-loja desde o início).

**Botão de sair** (pedido explícito do usuário — antes só `js/layout.js` tinha): `fetchShopeeStaffUser()` chama `/auth/staff/me` (mesmo contrato de `layout.js`); se houver sessão, o topbar mostra usuário logado + botão "Sair" (`initShopeeLogout()` → `POST /auth/staff/logout` + redireciona pra `/pages/login.html`). Sem sessão (gate desligado) o botão simplesmente não aparece. Resolve o gap de acesso do papel `shopee-demo` (reviewer da Shopee), que antes não tinha como encerrar a própria sessão pela tela. Mesmo botão replicado em `js/layout-tiktok.js` (`fetchTiktokStaffUser`/`buildTiktokLogoutButton`/`initTiktokLogout`, resolve o mesmo gap pro papel `tiktok-demo`). `js/layout-amazon.js` ainda não tem esse botão (mesmo gap, não corrigido ainda — hoje não existe papel `amazon-demo`, então não é urgente).
...
- **Relatório do Dia**: fechamento do período em PDF, com os botões **Dia / Semana / Mês** no topo da própria aba (semana = **segunda a sexta**). Uma chamada só (`DB.getRelatorioPeriodo({from, to, store_id, marketplace})`) traz KPIs, séries, breakdowns e produtos; os gráficos Chart.js e as tabelas são montados sobre esse payload. O botão "Baixar PDF" reusa as mesmas tabelas e reexporta os gráficos como PNG em tema claro (`chart.toBase64Image()`) numa janela `window.open` que chama `window.print()` — mesmo padrão de DRE/Recebíveis, sem lib de PDF. Detalhes em `embalagem.md`.

`DB.getPedidoPorEtiqueta(shippingId)`/`finalizarEmbalagem(formData)`/`getVideosEmbalagem(params)`/`videoEmbalagemUrl(id)`/`getEmbalagemPorHora(params)`/`getRelatorioPeriodo(params)` — `finalizarEmbalagem` usa `DB._postForm()` (helper próprio, `fetch` com `FormData` cru, sem `Content-Type` manual — o navegador define o boundary do multipart sozinho; diferente do `_post` padrão, que sempre serializa `JSON.stringify`).
...
- **Alertas globais** (`initAlerts`): toca um beep (Web Audio API, sem arquivo externo) e mostra um toast + notificação nativa do browser quando chegam os eventos WS `question_received` (status `UNANSWERED`) e `message_received`. Também injeta um badge vermelho no link "Perguntas" da sidebar.
  ...
  - **Áudio + política de autoplay**: os navegadores criam o `AudioContext` no estado `suspended` — sem um gesto do usuário antes, nada toca. `initAlerts` mantém **um único `AudioContext` compartilhado** (`_getCtx`), destravado no 1º clique/tecla/toque da página **e** por um **botão explícito "🔔 Ativar som" no topbar** (`#btnSomAlerta`, sino cortado→sino verde ao ativar, toca uma confirmação). Criar um contexto novo por beep (como era antes) nascia sempre mudo — esse era o bug de "sem áudio".
