...
- **Sistema**: mcp (chat com IA), monitor (métricas de servidor + config Telegram), schedule (jobs agendados), webhook (logs de webhooks recebidos), saude-sistema (filas BullMQ + heartbeat dos processos + último webhook/sync, auto-refresh 15s — ver `workers.md`)
