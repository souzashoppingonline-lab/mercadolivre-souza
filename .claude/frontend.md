...
- **Módulos** (fora do menu Operacional — sistema próprio via switcher no topbar, ver `modules.md`): financeiro (`pages/financeiro.html`, layout `js/layout-financeiro.js`, "em construção"), inteligencia-negocio (layout `js/layout-bi.js`) — `inteligencia-negocio.html` (Painel Estratégico, "em construção"), `bi-vendas.html` (Resumo por Venda), e as 6 páginas de Inteligência de Margem (`bi-margem.html` Visão Geral + `bi-margem-produtos.html` + `bi-margem-portfolio.html` + `bi-margem-frete.html` + `bi-margem-estoque.html` + `bi-margem-acoes.html`, navegação entre elas via `js/biMargemTabs.js` — detalhe completo em `modules.md`)
