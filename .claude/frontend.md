...
- **Modal de detalhe/edição**: abre ao clicar num cartão — mostra origem/loja/link do anúncio (se `metadata.link` existir, vindo de um cartão automático), permite editar todos os campos + mover de coluna, e lista/adiciona comentários (`DB.getTaskComments`/`DB.addTaskComment`). Para cartões automáticos, um bloco extra ("Detalhes — Estoque Crítico"/"Detalhes — Score de Qualidade", função `autoInfoHtml()`) renderiza os campos de `metadata` específicos da regra que gerou o cartão (SKU/quantidade ou score atual/lista de problemas) — não aparece em cartões manuais, que não têm esses campos em `metadata`.
