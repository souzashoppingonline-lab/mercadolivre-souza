...
## Topbar em resoluções médias (1024px) ainda estoura — gap residual não corrigido
...
## `GET /items/:item_id/promotion` (routes/api.js, seção 6) nunca de fato lê `seller-promotions/users/:id/promotions`

Descoberto testando "Adicionar Promoções" (v108) ao vivo em produção: o path usado nessa seção da rota (`/seller-promotions/${uid}/promotions?status=started...` e `/seller-promotions/${promo.id}/items...`) **não existe** na API real — o ML responde 404 estruturado (`"Resource .../promotions not found"`). Como esse trecho está dentro de um `try { } catch {}` mudo, o erro nunca aparecia — a rota "funciona" porque tem 5 outras fontes redundantes (sale_price, item.promotions, deal_ids, `/items/:id/promotions`) cobrindo a maioria dos casos reais, então ninguém notou que essa fonte específica sempre falhava silenciosamente. Path correto confirmado ao vivo: `/seller-promotions/users/:id?app_version=v2` (sem `/promotions`) e `/seller-promotions/promotions/:promotionId/items` (com `promotions` no meio) — já corrigidos em `mlClient.listSellerPromotions`/`getPromotionItems` (usados pela aba nova). **Não corrigido nesta tarefa**: a seção 6 de `GET /items/:item_id/promotion` continua com o path antigo (baixo risco — é só uma fonte a mais que já falhava sempre, sem afetar o resultado final). Corrigir se algum dia essa fonte fizer falta.
...
## "Custo Fixo" existe em DOIS lugares do Financeiro, só um alimenta o DRE

`pages/financeiro-despesas.html` (tabela `expenses`, `type='fixed'`) e `pages/financeiro-boletos.html` (tabela `boletos_mensais`, `tipo='custo_fixo'`) têm um botão/segmento com o **mesmo rótulo "Custo Fixo"**, mas gravam em tabelas diferentes. `pages/financeiro-dre.html` lê **só** `expenses`/`sales_entries`/`expense_categories` (client-side, `expMonth()`/`monthMetrics()`) — um lançamento feito pela tela de Boletos nunca aparece no DRE, nem na Projeção de Caixa (que também lê `expenses` pra custo fixo/operacional do mês), mesmo com o tipo "certo" selecionado. Investigado depois do usuário reportar um lançamento de Vale Refeição (Custo Fixo) que sumiu no DRE — o cálculo do DRE em si (`monthMetrics()`) não filtra por `type`, então qualquer despesa com `month`/`year` corretos aparece independente de fixo/operacional; a causa mais provável é ter usado a tela errada (ou a competência/mês selecionado no lançamento não bater com o mês que o DRE está mostrando). Script de diagnóstico: `node server/scripts/verificarDespesa.js "<termo da descrição>"` — mostra em qual das duas tabelas o lançamento caiu.

**Correção esperada:** renomear um dos dois rótulos pra deixar claro o destino (ex.: Boletos vira "Custo Fixo (conta a pagar)" só pra controle de vencimento) OU, se fizer sentido pro negócio, uma "Custo Fixo" da tela de Boletos também gravar um espelho em `expenses` pra entrar no DRE automaticamente. Nenhuma das duas foi feita ainda — decisão de produto pendente do usuário.

## CORRIGIDO — `dotenv.config()` em `config/env.js` usava caminho relativo ao cwd, não ao arquivo

Descoberto rodando `server/scripts/verificarDespesa.js` (acima) de `/opt/ml-dashboard-novo` (raiz do repo): `env.financeiro.*` veio tudo `undefined`, mesmo com o `.env` de produção configurado — porque `dotenv.config()` sem `path` explícito resolve `.env` relativo a `process.cwd()`, e o `.env` real fica em `server/.env` (`.claude/deployment.md`), não na raiz. O fluxo normal (`systemd`/`npm start`, cwd sempre `server/`) nunca expôs isso; qualquer script standalone (`server/scripts/*.js`) rodado de outro diretório ficava com toda a configuração vazia, silenciosamente (dotenv não avisa se o arquivo não existe). **Corrigido**: `config/env.js` agora ancora o caminho em `__dirname` (`path.join(__dirname, '..', '..', '.env')`), resolve pra `server/.env` não importa de onde o comando for rodado.

## TikTok Shop: pedido `COMPLETED` pode ser "entregue" OU "reembolsado 100%" — hoje os dois viram 'paid'

A doc oficial (Order API overview) confirma que `COMPLETED` é alcançado por dois caminhos bem diferentes: o normal (`DELIVERED → COMPLETED`, entrega concluída) e um atalho (`AWAITING_COLLECTION`/`IN_TRANSIT → COMPLETED` quando o pedido é 100% reembolsado antes de chegar ao comprador). `mapTiktokStatus()` (`marketplaceEventWorker.js`) trata todo `COMPLETED` como `'paid'`, sem diferenciar os dois casos — um pedido totalmente reembolsado nesse 2º caminho conta como venda normal nos KPIs, inflando levemente a receita.

**Correção esperada**: consultar `order/{v}/orders/{id}/price_detail` (Order API, confirmado na doc) ou a Return & Refund API pra saber se houve reembolso total antes de contar o pedido como `'paid'` — fora do escopo da Fase 1 ("só vendas"), que ainda não lê nenhum desses dois endpoints. Baixo impacto esperado (reembolso 100% antes da entrega não deve ser o caminho comum), mas fica registrado pra não ser redescoberto.

## CORRIGIDO (só em `tiktok-vendas.html`) — falha no Chart.js travava a tabela de pedidos, sem relação nenhuma com gráfico

Descoberto gerando capturas de tela reais da página pra submissão do TikTok Partner Center: com o CDN do Chart.js bloqueado/indisponível, `drawDia()` lança uma exceção não tratada (`Chart is not defined`) dentro de `loadAll()` — como `drawStatus()` e `await loadPedidos()` vêm DEPOIS na mesma função, sem try/catch, a exceção aborta a função inteira. Resultado: os KPIs (que já tinham sido preenchidos antes) ficam certos, mas a tabela "Últimos pedidos" — que não depende do Chart.js pra nada — fica travada no spinner de carregamento pra sempre, sem nenhuma mensagem de erro. **Corrigido em `tiktok-vendas.html`**: `drawDia`/`drawStatus` agora rodam cada um no próprio try/catch dentro de `loadAll()`.

**CORRIGIDO também em `shopee-vendas.html`** (nova rodada de capturas de tela, mesmo motivo): `drawDia`/`drawStatus`/`drawLoja` ganharam o mesmo try/catch individual dentro de `loadAll()` — a tabela "por loja" não trava mais se o Chart.js falhar.

**Ainda não corrigido em `vendas.html`/`amazon-vendas.html`**, que seguem o mesmo molde (não auditados nesta tarefa). **Correção esperada**: mesmo try/catch por gráfico quando alguém mexer nelas de novo, ou extrair um helper único (`drawChartSafe(fn)`) pra não repetir o padrão em cada arquivo.

## CORRIGIDO — `d.dia` concatenado direto com `'T12:00:00'` gerava "Invalid Date" no eixo do gráfico "Vendas por dia"

Descoberto na mesma rodada de capturas de tela do TikTok Partner Center: `drawDia()` (`tiktok-vendas.html` e `shopee-vendas.html`, código idêntico) fazia `new Date(d.dia + 'T12:00:00')`, esperando `d.dia` no formato `"AAAA-MM-DD"`. Só que a query (`routes/tiktok.js`/`routes/shopee.js`, `(o.date_created AT TIME ZONE 'America/Sao_Paulo')::date AS dia`) devolve um `date` do Postgres, que o driver `pg` serializa como timestamp ISO completo (`"2026-08-19T00:00:00.000Z"`) — a concatenação virava `"...000ZT12:00:00"`, uma data inválida, e todo o eixo X do gráfico de vendas por dia mostrava "Invalid Date" em vez da data. **Corrigido**: `new Date(String(d.dia).slice(0, 10) + 'T12:00:00')` — mesmo padrão de parsing já usado em `pages/vendas-por-loja.html` (`diaStr`), não é uma fórmula nova.

**Mesmo padrão também em `pages/produtos.html`** (descoberto testando a Observação de Embalagem, v101): `openProdModal()` monta `content.innerHTML` (header, Observação de Embalagem, KPIs) e só DEPOIS chama o `new Chart(...)` do gráfico de vendas diárias, sem try/catch — com o CDN do Chart.js bloqueado, a função async lança e aborta, mas como o `innerHTML` já foi setado antes, o modal fica **utilizável** (só falta o gráfico) — impacto bem menor que em `tiktok-vendas.html`/`shopee-vendas.html`, onde a tabela inteira travava no spinner. Mesma correção esperada quando alguém mexer no arquivo de novo.
