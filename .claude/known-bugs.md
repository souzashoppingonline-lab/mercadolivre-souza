...
**Mesmo padrão (não corrigido ainda) em `shopee-vendas.html`** (`drawDia`/`drawStatus`/`drawLoja` também sem try/catch antes da tabela por loja) — e possivelmente em `vendas.html`/`amazon-vendas.html`, que seguem o mesmo molde (não auditados nesta tarefa, escopo era só TikTok). **Correção esperada**: aplicar o mesmo try/catch por gráfico nessas páginas quando alguém mexer nelas de novo, ou extrair um helper único (`drawChartSafe(fn)`) pra não repetir o padrão em cada arquivo.
