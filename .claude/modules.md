...
- **Vendas & Custos** (`pages/financeiro-vendas.html`, nav `Vendas & Custos`): lê `sales_entries` + `stores` + `monthly_goals`. **Funcionalidade fiel ao Readdy** (tema escuro nosso): **Meta de Faturamento** (11 cards — Realizado D-1, Meta, Meta Restante, Gap Projeção, Velocidade Média, Projeção Mês, Meta do Dia, Ontem, Pior Dia, Meta Mínima, Dia Ideal — via porta do `useGoalCalculations`), barra de progresso + status ahead/risco/atrasado; **Metas por Empresa** (card por loja: realizado/meta/%/projeção/ritmo/margem); **Indicadores do Período** (Receita, MC, Vendas, Ticket + variação vs mês ant.); **Meta Diária** (dia a dia com semáforo Passou/Faltou, meta = goal/dias); gráfico margem % diária + donut custos; **Relatórios por Empresa** (composição de custos em barras). **Formulário "Novo Lançamento Diário"** (escrita): grava uma linha em `sales_entries` (data, qtd, receita, taxas, frete, CMV, ads ML, ads externos, imposto, loja) e reflete na tela sem recarregar — 1ª tela com escrita da migração.
