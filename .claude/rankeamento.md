...
**Filtro por status de Buy Box (v103, pedido do usuário):** na aba Catálogo, acima dos cards, uma barra de pílulas (`Todos` / `🏆 Ganhando` / `🤝 Compartilhando o 1º lugar` / `📉 Perdendo`), cada uma com a contagem ao lado. Client-side, sem request nova — `loadCards()` guarda a lista completa em `CATALOGO_ADS` (`rankeamento.html`), e clicar numa pílula (`setFiltroBuyBox(chave)`) refiltra e rerenderiza (`renderCatalogoFiltrado()`) sem tocar a API. `statusBuyBox(a)` (a mesma função que decide o texto/cor do card, extraída pra escopo de módulo pra não duplicar a regra) ganhou um campo `chave` estável (`ganhando`/`compartilhando`/`perdendo`/`aguardando`/`sem_concorrencia`) usado só pelo filtro — o texto exibido no card continua vindo de `label`. "Aguardando 1ª sincronização" e "Sem concorrência detectada" não têm pílula própria (ficam visíveis só em "Todos") por não serem um dos 3 status que o usuário pediu para filtrar. Otimização junto: `renderPanel()` agora pula a busca de eventos (`GET /ranking/:id/eventos`) pra anúncios já cacheados em `EVENTS[a.id]`, evitando refazer as mesmas chamadas de rede a cada troca de filtro.
...
**Relatório diário no Telegram (pedido do usuário — 08:00, `relatorioRankeamentoDiario`, `worker.js`):** reusa `computarRankeamento()` pro dia de ONTEM — mesmo dado de `bi-rankeamento.html`/relatório em PDF, nunca uma 2ª fórmula. Além do que já vem pronto (`por_fase_periodo`/`produtos_por_estagio`), calcula à parte quem **não vendeu** ontem entre os anúncios ativos no módulo (`ranking_ads WHERE active=true` menos quem aparece em `produtos_por_estagio` — esse cálculo só lista quem teve venda, não é um catálogo). Detalhes/schedule em `workers.md`. `computarRankeamento`/`FASE_LABEL_PT`/`FASES_RANKEAMENTO` foram exportados de `routes/bi.js` (mesmo padrão já usado por `computarMargem`, consumido pelo Agente Financeiro) pra esse reaproveitamento.
