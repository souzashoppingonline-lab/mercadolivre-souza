...
## Ciclos de rankeamento + métricas de ADS (v72)

Cada anúncio pode passar por **vários ciclos** de rankeamento (campanhas/empurrões sucessivos). O card mostra o **Ciclo N** atual (badge 🔄) e três campos **preenchidos manualmente** — ADS investido (R$), ROAS e orçamento diário (R$) — porque o ML não expõe esses números por webhook. Colunas em `ranking_ads`: `ciclo`, `ads_investido`, `roas`, `orcamento_diario`, `ciclo_iniciado_em` (ver `database.md`).

- **Salvar manual:** cada input salva no `onchange` via `PATCH /api/ranking/ads/:id` (`DB.patchRankingAd`) — sem re-render, pra não perder o foco. O card exibe um **ROAS calculado** (dica) = faturamento do ciclo ÷ ADS, mas o campo ROAS continua manual (fonte de verdade).
- **Faturamento por ciclo:** o `faturamento` de `/ads` e o do marco (`ranking.js` → `milestone`) passam a somar só as vendas `>= ciclo_iniciado_em` (o card e o Telegram falam do ciclo corrente, não do acumulado histórico). Cards antigos: `ciclo_iniciado_em` foi backfillado com `started_at`, então nada muda pra eles (ficam no ciclo 1).
- **Novo ciclo** (botão 🔁 no card → `POST /api/ranking/ads/:id/ciclo`, `DB.novoCicloRankingAd`): arquiva os números do ciclo atual (ADS/ROAS/orçamento + `sales_count` + faturamento) em `ranking_ciclos`, incrementa `ciclo`, **zera** o contador (`sales_count`/`first_sale_at`/`last_sale_at`) e os manuais, e carimba novo `ciclo_iniciado_em`. O contador 5-em-5 e o ritmo reiniciam pro novo empurrão.
- **Histórico:** `<details>` "Ciclos anteriores (N)" no rodapé do card carrega sob demanda via `GET /api/ranking/ads/:id/ciclos` (`DB.getRankingCiclos`) — tabela com vendas/faturamento/ADS/ROAS/orçamento de cada ciclo encerrado. Só aparece se `ciclos_anteriores > 0`.
