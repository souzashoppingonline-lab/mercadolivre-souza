...
**Implementado (v44)**: colunas `net_commission_fee`, `net_service_fee`, `service_fee`, `seller_product_rebate` em `shopee_order_data`, populadas pelo worker (`getEscrowDetail` → `order_income`) e **backfilladas do `escrow_raw`** já persistido (migration v44 faz `UPDATE ... FROM escrow_raw->'order_income'`, sem re-chamar a API). O cálculo de taxa passou a usar o **líquido**: fragmento SQL `TAXA_NET_SQL` (`routes/shopee.js`) = `net_commission_fee + net_service_fee` quando presentes, **senão** cai no bruto `commission_fee` (pedidos anteriores à mudança). Usado em `escrowFeePct()` (taxa automática do Precificador/Executivo/Performance) e na rota `/api/shopee/financeiro` (coluna Taxa por pedido + KPI comissão). O **líquido/repasse** (`escrow_amount`) já era o valor real da Shopee e não mudou. Regra oficial: **sempre considerar a resposta completa da API** e acompanhar os anúncios no console do desenvolvedor.
...
## Promoções — implementado (tela própria + alerta Telegram/e-mail granular)
...
- **Job `syncShopeePromos`** (`marketplaceEventWorker`, a cada `SHOPEE_PROMO_INTERVAL_MS`=1h): busca descontos (ongoing+upcoming) e vouchers (não expirados), **só faz upsert** em `shopee_promotions` — desde v95 não alerta mais aqui (ver abaixo, motivo: uma vez vencida a Shopee para de devolver a campanha em `ongoing`/`upcoming`, então um alerta de vencimento dentro deste sync nunca chegaria a avisar "já venceu").
- **Alerta de vencimento** (`checkShopeeCampanhasVencendo`, `worker.js`, **1x/dia às 06:35** — v95): lê `shopee_promotions` direto (não depende do que a API Shopee ainda lista como ongoing/upcoming), calcula dias restantes em **dias de calendário America/Sao_Paulo** (não horas) e avisa por **Telegram** (`tg_shopee_campanhas`) **e e-mail** (`email_shopee_campanhas`, toggle em `/config/email`) quando faltam **5, 4, 3, 2 ou 1 dia** pro fim (1 aviso por degrau cruzado, nunca repete o mesmo) e, depois de vencida, **1x por dia** enquanto durar (limitado a campanhas vencidas há no máx. `SHOPEE_PROMO_EXPIRED_ALERT_DAYS`=30 dias, pra não virar alerta eterno sobre campanha antiga esquecida). Dedup via `shopee_promotions.expiry_alert_min_days`/`expiry_alert_expired_date` (substituíram o antigo `expiry_notified`, único e <24h/Telegram-apenas). Rearma (volta a `NULL`) se a campanha for estendida pra mais de 5 dias de novo. Todos os avisos do dia saem **consolidados em 1 mensagem/1 e-mail**, não 1 por campanha. Disparo manual: `worker:cmd` `shopee-campanhas-vencendo`/`checkShopeeCampanhasVencendo`.
...
- Tabela `shopee_promotions` (v41, colunas de alerta atualizadas na v95) — ver `database.md`.
...
- **Preço atual 0/ausente = não sincronizado, não é R$0,00 real** (bug de produção corrigido): `shopee_item_data.models[].current_price` pode vir `0`/`null` quando o catálogo ainda não sincronizou aquele anúncio (job roda a cada 30min — ver `workers.md`). Tratar `0` como preço válido zerava a simulação inteira (comissão/imposto/lucro/margem todos "R$ 0,00" em vez de "sem dado"). As duas rotas abaixo normalizam `current_price <= 0` → `null` e devolvem `current_price_sincronizado: false`; o frontend (`renderModal`) mostra "—" nesses campos e um aviso, em vez da conta com preço zerado. **Cuidado ao mexer em `/precificador/simular`**: o `...calc` (retorno de `calculateShopeePricing`) sempre inclui uma chave `currentPrice` (camelCase) igual a `toReais(preçoInválido)` = `0` — é preciso sobrescrever essa chave explicitamente **depois** do spread com o valor já normalizado, senão o frontend (que lê `d.currentPrice`, não `d.current_price`) volta a ver `0`.
  - **2ª rodada (mesmo dia)** — depois desse fix, a simulação inteira ficava em branco (comissão/taxa fixa/imposto/lucro "—") sempre que faltava o preço atual, mesmo já existindo um preço IDEAL calculado com faixa/comissão bem definida pra ele — reportado pelo usuário como regressão ("preço atual sumiu", "tarifas/impostos/comissão não aparece"). Como o **preço ideal É o preço que vai ser anunciado** (definição do próprio usuário — ver acima), `/precificador/simular` agora também devolve `commission_rate_ideal`/`fixed_fee_ideal`/`net_revenue_ideal` (comissão/taxa fixa/receita líquida **no preço ideal**, calculados de `calcNoIdeal`, ao lado de `margin_no_ideal`/`profit_no_ideal` que já existiam). O modal (`renderModal`) usa esses campos como fallback: sem preço atual sincronizado, a "Simulação da venda" roda em cima do **preço ideal** (rotulado "Preço anunciado (preço ideal)"), não fica em branco — só o card "PREÇO ATUAL" continua "—" (isso sim é informação real que falta). Os cards de topo (Taxa Shopee/Taxa fixa) seguem a mesma regra.
  - **Preço ideal sobe quando o % de cupom sobe — isso é esperado, não bug**: como o preço ideal precisa "absorver" o desconto (seção acima), dar mais cupom exige um preço de tabela mais alto pra sobrar a mesma margem depois do desconto — o efeito é super-linear porque cupom loja+produto empilham multiplicativamente E a comissão também incide sobre o preço pós-desconto (mais preço = mais comissão em R$, exigindo subir ainda mais o preço ideal). Verificado numericamente (`node -e` com `calculateShopeePricing`) antes de descartar como bug.
...
**Frontend**: cards de configuração (Margem desejada, editável; Taxa Shopee/Taxa fixa, somente exibição — refletem a faixa do anúncio aberto no modal, já que uma lista com preços variados não tem uma "taxa única" que faça sentido mostrar; Imposto, editável) + tabela oficial de comissão (renderizada a partir de `tiers`, clicável) + lista de anúncios (custo editável inline, "aplicar" rápido por variação — fluxo que já existia, mantido) + botão **"Atualizar anúncios"** (força o sync do catálogo agora — ver `workers.md`) + modal ao clicar na linha, que é **só calculadora** (pedido do usuário: simula margem com promoção relâmpago/cupom loja/cupom produto sem precisar aplicar nada ali — quem aplica é o botão "aplicar" da lista, fora do modal), com simulação de 12 linhas e margem/lucro atual × no preço ideal com selo "MARGEM DENTRO DA META"/"MARGEM DO ANÚNCIO APÓS CUPOM" (nome do 2º selo trocado a pedido do usuário — era "MARGEM ABAIXO DA META").
- **Faixa forçada é POR VARIAÇÃO, não pra lista inteira** (bug relatado: "escolher a faixa aplica em todos"): clicar numa linha da tabela oficial de comissão não força mais o preço daquela faixa em toda a lista automaticamente. `GET /precificador` sempre devolve os dois preços por variação — `suggested_price` (automático, a melhor faixa) e `suggested_price_faixa`/`suggested_price_faixa_within_tier` (o preço forçado nessa faixa, calculado sempre que uma faixa está selecionada, mas sem aplicar sozinho). Cada linha da lista ganhou um **checkbox** (+ um "marcar todos" no cabeçalho); o frontend só usa `suggested_price_faixa` na coluna "Preço ideal"/botão "aplicar" das variações **marcadas** — as demais continuam com `suggested_price` automático. Marcar/desmarcar reusa o último resultado já buscado (`lastData`), sem chamada nova ao backend — só clicar numa faixa diferente busca de novo (o preço forçado daquela faixa ainda não foi calculado). O modal **nunca** usa a faixa forçada (sempre calcula pela faixa real do preço atual/ideal do anúncio, ver `priceForTier` só na rota de lista).
...
## CORRIGIDO (12/09/2026) — `trust proxy` ausente rejeitava 100% dos webhooks

`server.js` roda atrás do nginx (SSL termination). Sem `app.set('trust proxy', 1)`, `req.protocol` do Express sempre voltava `'http'` (a conexão real nginx→Node), mesmo a nginx já mandando `X-Forwarded-Proto: $scheme` certinho. `shopeeWebhook.js` monta a URL usada na verificação HMAC (`pushUrl = ${req.protocol}://${req.get('host')}${req.originalUrl}`) — vinha sempre `http://...`, enquanto a Shopee assina em cima da URL real `https://...`. Resultado: **toda** assinatura batia errada, todo webhook Shopee era rejeitado silenciosamente (log `[shopee-webhook] assinatura não confere`), sem exceção — o sistema rodava só no polling de 15min como se fosse a via principal (era pra ser rede de segurança). Explica os casos reportados de "etiqueta bipada não encontrada" pra pedido recém-criado. Corrigido com `app.set('trust proxy', 1)` em `server.js`. Nenhum outro webhook (ML, TikTok) dependia de `req.protocol` pra assinatura. **Narrativa completa da investigação** (essa causa + a 2ª causa relacionada, do fallback de bipagem que não achava pedido recém-criado mesmo com webhook já correto) está em `embalagem.md`, seção "Investigação completa (08–12/09/2026)" — não repetida aqui.
