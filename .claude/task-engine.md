...
- **Regras 4 e 5 (promoção ativa / promoção vencendo) — REMOVIDAS (v105)**: existiram entre v97 e v104 (`checkPromoAtiva`/`checkPromoVencendo`, `rule_key` `promocao_ativa_shopee`/`promocao_vencendo_shopee`). Pedido do usuário: promoção não é uma tarefa/problema, só um estado do anúncio — os cartões poluíam o quadro sem necessidade. `shopee_promotions` continua sincronizado normal (`syncShopeePromos`) e o alerta de vencimento por Telegram/e-mail (`checkShopeeCampanhasVencendo`) continua intacto — nenhum dos dois nunca dependeu do TaskEngine, só paravam de *também* criar um cartão. Se a regra 5 (Telegram/e-mail) algum dia sumir, é bug separado — não relacionado a esta remoção.
...
- **`syncShopeePromos`** (`marketplaceEventWorker`, 1h) e **`checkShopeeCampanhasVencendo`** (`worker.js`, 1x/dia) continuam rodando (sincronização + alerta Telegram/e-mail), mas **não chamam mais o TaskEngine** desde a v105 — ver "Regras 4 e 5" acima.
