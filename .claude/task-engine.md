...
- **Regra 4 (v97, Shopee, rótulo corrigido na v99) — `checkPromoAtiva`** (`rule_key='promocao_ativa_shopee'`): cartão informativo, prioridade `baixa`, criado pra toda campanha `shopee_promotions` que está `ongoing`. Título distingue por tipo: `flash_sale` → "Oferta Relâmpago ativa" (Shop Flash Sale, API própria integrada na v99 — ver `shopee.md`); `discount` → "Desconto ativa"; `voucher` → "Cupom ativo". **v99 corrige um mapeamento errado**: até a v97/v98, `discount` (`getDiscountList`) era rotulado como "oferta relâmpago" por ser o mais próximo do que já estava sincronizado — o usuário apontou que é uma campanha diferente dentro da própria Shopee (preço promocional comum, sem vitrine/estoque reservado) e pediu a API real. Chamada por `syncShopeePromos()` (`marketplaceEventWorker`, 1h) pra cada promo recém-sincronizada como ongoing.
