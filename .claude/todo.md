...
- [x] ~~Avaliar se `MAX_ADS=30` ainda faz sentido~~ — resolvido: removido por completo (todos os estágios, não só catálogo), pedido explícito do usuário depois de esbarrar nele de verdade. Ver `decisions.md`.
- [ ] **Observar em produção se o snapshot de 6h (`sync-ranking`) aguenta o portfólio sem o teto** — trade-off assumido ao remover `MAX_ADS`: mais anúncios rastreados = mais chamadas à API do ML por rodada. O job já tem throttling/circuit-breaker próprio; se aparecer 429 recorrente, ajustar o throttling do job (nunca trazer o teto de volta sem pedir de novo). Ver `decisions.md`.
...
## Analista Ecom (v90-v93, `pages/analista-ecom.html`)
