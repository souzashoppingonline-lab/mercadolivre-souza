...
- [ ] **Avaliar se `MAX_ADS=30` ainda faz sentido pro portfólio do usuário** — incidente real: o usuário reportava "mudar o estágio não pega" repetidamente; a causa raiz era o teto de 30 anúncios (fora catálogo) sendo atingido silenciosamente (bug de `addAd()` engolindo o erro, corrigido — ver `decisions.md`). Com o alerta agora aparecendo de verdade, se o portfólio dele precisar de mais de 30 simultâneos, subir o teto tem custo real (mais chamadas à API do ML no snapshot de 6h) — decisão do usuário, não fazer sozinho.
