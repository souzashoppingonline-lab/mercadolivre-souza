...
| a cada 4h (junto com `top-vendas`) | `syncShippingStatus` | Conciliação Bancária (Status de Entrega, v33): reconsulta `/shipments/:id` de até 200 pedidos com `shipping_id` conhecido e `shipping_status` ainda não terminal (`delivered`/`cancelled`), últimos 45 dias, priorizando os com `shipping_last_updated` mais antiga. Necessário porque o webhook `shipments` não garante 1 evento por transição de status — sem esse job, `shipping_status` fica travado no último valor recebido (ex.: preso em `pending`/"Aguardando envio" mesmo depois do pedido já ter sido enviado de verdade) — mesmo racional de `sync-payment-releases`. Ver `decisions.md` |
