...
| `payments` | `handlePayment` | busca `/collections/:id`, extrai `order_id`, delega para `handleOrder`; a partir da v29/v30, também persiste o retorno completo em `ml_payments` (status, valor líquido, `money_release_date`, `released`, taxas detalhadas) — Conciliação Bancária, ver `conciliacao-bancaria.md` e `database.md` |
...
| `post_purchase` | `handlePostPurchase` | busca `/post-purchase/v1/claims/:id` (`ml.getClaim`), upsert em `returns` — `order_id` vem de `claim.resource_id` quando `claim.resource === 'order'` (a claim não tem campo `order_id` de verdade, ver `decisions.md`), publica `devolucao_recebida`, notifica Telegram (`tg_devolucoes`) se `status='opened'` |
...
| `shipments` | `handleShipment` | busca `/shipments/:id` (`ml.getShipment`), persiste `shipping_status`/`shipping_substatus`/`date_ready_to_ship`/`date_shipped`/`date_delivered`/`shipping_last_updated` em `orders`, casando por `orders.shipping_id` (v33 — antes casava por `raw_data->>'shipment_id'` e só reprocessava o pedido inteiro via `handleOrder`, que raramente refletia mudança de status de entrega; ver `decisions.md`). UPDATE que não casa nenhuma linha (pedido ainda não existe localmente) é ignorada — Conciliação Bancária, ver `conciliacao-bancaria.md` |
...
| 04:30 | `syncSeoScore` | calcula o SEO Score (`server/src/seoScore.js`, fórmula determinística, **nunca IA** — ver `decisions.md`) de itens ativos por loja ML, grava em `item_seo_score` (upsert) + `item_seo_score_history` (append). Reaproveita `ml.getItem()` já feito por outros jobs quando possível, chama `ml.getItemDescription()` (contagem de palavras) e `getRequiredAttrsForCategory()` (cache de 30 dias em `category_attributes_cache`, ver `database.md`); visitas/vendas 30d vêm de Maps pré-batched por loja (1 query cada, não por item — evita N+1). Mesmo padrão de throttling de `syncScores`: 10s entre itens, pausa 30s a cada 10, circuit breaker de 5 429 seguidos aborta a loja |
| 04:50 | `syncCatalogCompetition` | Monitor de Buy-Box (catálogo): 1 chamada `price_to_win` por item `catalog_listing=true` já conhecido via `item_seo_score` (v25) — não busca lista de concorrentes aqui (isso é sob demanda, ver `api.md`). Grava `catalog_competition` (upsert) + `catalog_competition_history` (append). Mesmo padrão de throttling/circuit-breaker de `sync-seo-score`. Ver `decisions.md` (por que não usa busca por palavra-chave — `/sites/MLB/search` bloqueada) |
| 05:00 | `syncPrecos` | atualiza `items.original_price` de itens não fechados; zera se não há promoção ativa |
| 05:15 | `syncPaymentReleases` | Conciliação Bancária: reconsulta (`/collections/:id`) até 200 pagamentos com `ml_payments.released != 'yes'`, priorizando `money_release_date` mais próxima — a liberação acontece semanas depois da aprovação e não necessariamente gera um novo webhook `payments`, então sem esse job `released` ficaria travado em `'no'` mesmo depois do dinheiro ser liberado de verdade. Ver `conciliacao-bancaria.md` |
| 05:25 | `checkConciliacaoDivergencias` | Conciliação Bancária: 1 alerta Telegram consolidado (`tg_conciliacao`) pra 2 tipos de divergência — diferença bruto/líquido ≥50% (`CONCILIACAO_DIFERENCA_ALERTA_PCT`) ou liberação atrasada +2 dias (`CONCILIACAO_LIBERACAO_ATRASO_DIAS`) — limiares fixos iniciais, ver `business-rules.md`. Dedup via `ml_payments.alert_notified_at` (v32, mesmo padrão de `tasks.overdue_notified_at`) — notifica 1x por pagamento |
| 05:40 | `mp-reports` (`syncMpAccountReports`, `mpReports.js`) | Conciliação fase 2: baixa os Relatórios de Liberação (`release_report`) do Mercado Pago com o token do ML (autentica em `api.mercadopago.com`, ver `decisions.md`), parseia o XLSX (`xlsx`) e grava cada movimento da conta em `mp_account_movements` (extrato: liberações, saques `Cash withdrawal`, estornos, disputas). Casa por `source_id`=payment_id / `order_id`. Idempotente: cada arquivo importado 1x (`mp_reports_imported`) + `movement_hash` único (dedup entre relatórios com range sobreposto). Máx 3 arquivos novos por execução (converge; a UNIFULL tem 67). Contas sem relatório agendado recebem 1 `createMpReport` sob demanda (últimos 60d). Comando manual: `mp-reports`/`syncMpAccountReports`. Ver `conciliacao-bancaria.md` |
| a cada 4h (junto com `top-vendas`) | `syncShippingStatus` | Conciliação Bancária (Status de Entrega, v33): reconsulta `/shipments/:id` de até 120 pedidos com `shipping_id` conhecido e `shipping_status` ainda não terminal (`delivered`/`cancelled`/`NULL`), últimos 45 dias, priorizando os com `shipping_last_updated` mais antiga. Intervalo de 8s entre chamadas (20s após um 429) — conservador porque a API do ML já está ocupada com webhooks em horário de pico. Necessário porque o webhook `shipments` não garante 1 evento por transição de status — sem esse job, `shipping_status` fica travado no último valor recebido, ou nunca chega a ser preenchido se nenhum webhook `shipments` foi recebido pra aquele pedido. Circuit breaker de 429 é **por loja** (3 erros seguidos da mesma loja pausam só ela nesta execução, via `Set`/`Map` locais à função) — um 429 isolado de 1 loja não pode travar as outras 199 ordens da fila só porque calhou de ser a 1ª loja processada (bug real corrigido, ver `decisions.md`). Ver `conciliacao-bancaria.md` |
...
| 06:30 | `checkTaxaDevolucaoAlta` | por loja ML, compara devoluções vs. pedidos das últimas 24h; lojas com `< 10` pedidos no dia são ignoradas (amostra pequena demais); taxa `≥ 5%` dispara alerta Telegram (`tg_devolucoes`, mesmo tópico já usado por "Nova devolução solicitada"). Mesma fórmula do card "Taxa de Devolução" de `pages/devolucoes.html`, mas em janela fixa de 24h, não o filtro livre da tela. Ver `business-rules.md` |
...
| 08:15 | `checkTarefasAtrasadas` | busca cartões da Agenda Trello com `due_date < now()` fora das colunas `finalizado`/`excluido` e ainda não notificados (`overdue_notified_at IS NULL`), envia lista ao Telegram (`tg_tarefas`) e marca `overdue_notified_at = now()` — evita repetir o alerta todo dia pro mesmo cartão; volta a `NULL` (renotifica) se o `due_date` for editado (`PATCH /api/tasks/:id`). Ver `business-rules.md` e `task-engine.md` |
...
- **Rastreio Shopee (v35)**: `handleShopeeOrderEvent`, quando o status está em `SHOPEE_SHIPPABLE` (`READY_TO_SHIP`/`PROCESSED`/`RETRY_SHIP`/`SHIPPED`/`TO_CONFIRM_RECEIVE`/`COMPLETED`), chama `ShopeeClient.getTrackingNumber` e grava em `shopee_order_data.tracking_number` (`COALESCE` no upsert — não apaga um valor já gravado se vier vazio). É o `BR…` do QR da etiqueta, usado pela Embalagem (estação única ML+Shopee). Pedidos anteriores à v35: `server/backfill-shopee-tracking.js` (roda uma vez).
- **Financeiro (escrow) + status de entrega Shopee (v36)**: no mesmo bloco `SHOPEE_SHIPPABLE`, `handleShopeeOrderEvent` também chama `getEscrowDetail` (grava `buyer_total`/`commission_fee`/`escrow_amount` líquido/`buyer_payment_method`/`escrow_raw`) e `getTrackingInfo` (grava `logistics_status`) em `shopee_order_data` — cada chamada tolerante a falha, `COALESCE` no upsert. Alimenta a página **Financeiro Shopee** (`/api/shopee/financeiro`). Backfill dos antigos: `server/backfill-shopee-financeiro.js`. São +2 chamadas por pedido embarcável; o volume é baixo, mas se virar backfill grande usar throttle (o script já espaça 500ms).
...
- **Notificação Telegram de vendas Shopee**: implementada. `handleShopeeOrderEvent`, ao detectar venda nova (`status='paid'` e status anterior ≠ `paid`), chama `tgNotify('tg_vendas', ...)` — **mesmo tópico das vendas ML**, então respeita o mesmo on/off, janela de silêncio e throttle já configurados. A mensagem marca `🛍️ Shopee` pra distinguir do ML. Pra isso o `tgNotify`/`tgNotifyForce` foram extraídos de `worker.js` pra `server/src/notify.js` (módulo compartilhado; `worker.js` passou a importar de lá — todos os call sites intactos). **Amazon** ainda não notifica (o handler Amazon não chama `tgNotify`; fica pra quando houver vendas reais — ver `todo.md`).
