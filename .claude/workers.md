...
## Eventos de outros marketplaces — v15/v16 (`marketplaceEventWorker.js`, processo à parte do dispatch acima)
...
- **Múltiplas contas (v16)**: no boot, `startMarketplaceEventWorkers()` busca todas as linhas de `stores` com `marketplace_id=AMAZON` e registra uma `AmazonPollingEventSource` (+ um `AmazonClient`, credenciais mescladas com override por conta) **por conta**, não uma instância fixa única. O mapa `clients` é chaveado por `stores.id`, não pelo código do marketplace.
- **Descoberta de eventos**: `server/src/marketplaces/Scheduler.js` orquestra todas as `EventSource`s registradas (`server/src/marketplaces/interfaces/EventSource.js`: `start/stop/discoverEvents`) — uma por conta Amazon, cada uma com polling a cada 15 min — a Amazon SP-API não tem webhook simples "topic+resource" como o ML.
- Cada execução de `discoverEvents()` lê o cursor `last_synced_at` de `marketplace_sync_state` (chave `source_key = stores.id` daquela conta; 24h atrás na 1ª execução), chama `AmazonClient.listRecentOrders`, e publica um evento por pedido na fila `marketplace-events-amazon` (`queues/marketplaceEventQueue.js`, compartilhada entre todas as contas), com `jobId` estável `AMAZON:{storeId}:ORDER_UPDATED:{amazonOrderId}` (mesma técnica anti-duplicata do Gateway ML, agora escopada por conta) e o campo `storeId` no payload identificando de qual conta veio.
- **Consumo**: o `Worker('marketplace-events-amazon', ...)` em `marketplaceEventWorker.js` usa `evt.storeId` para achar o `client` certo no mapa, busca o pedido completo (`AmazonClient.getOrder`), faz upsert em `orders` (com `marketplace_id`, `store_id = evt.storeId`) e nos campos exclusivos em `amazon_order_data`, invalida `kpis:summary` e publica `order_updated` no WS — mesmo formato que o pipeline ML usa, para as telas do dashboard não precisarem saber a origem.
...
- Notificação Telegram de vendas Amazon **não está implementada** (fora de escopo — ver `todo.md`).
...
| `/sync vendas\|metricas\|visitas\|devolucoes\|topvendas\|email-diario\|email-semanal` | dispara o sync correspondente |
