...
function mapAd(r) {
  return {
    ...
    comentarios_auto: r.comentarios_auto, descricao: r.descricao, highlights: r.highlights,
...
function adValues(b) {
  ...
  return {
    ...
    comentarios_auto: b.comentarios_auto || null, descricao: b.descricao || null,
    highlights: Array.isArray(b.highlights) && b.highlights.length ? JSON.stringify(b.highlights) : null,
...
async function upsertAd(productId, b) {
  ...
  const cols = 'product_id, ml_id, link, titulo, preco, preco_original, nota, vendas, perguntas, comentarios, vendedor, cidade, estado, reputacao, is_full, is_flex, fotos, videos, observacoes, comentarios_texto, comentarios_auto, descricao, highlights, vendas_7d, preco_medio_7d, vendas_15d, preco_medio_15d, vendas_21d, preco_medio_21d, vendas_30d, preco_medio_30d, raw';
  const vals = [productId, v.ml_id, v.link, v.titulo, v.preco, v.preco_original, v.nota, v.vendas, v.perguntas,
                v.comentarios, v.vendedor, v.cidade, v.estado, v.reputacao, v.is_full, v.is_flex, v.fotos, v.videos, v.observacoes, v.comentarios_texto, v.comentarios_auto, v.descricao, v.highlights,
  ...
  if (v.ml_id) {
    sql += ` ON CONFLICT (product_id, ml_id) DO UPDATE SET
               ...
               descricao=COALESCE(EXCLUDED.descricao, analise_product_ads.descricao),
               highlights=COALESCE(EXCLUDED.highlights, analise_product_ads.highlights),
