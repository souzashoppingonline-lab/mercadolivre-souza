...
//
// Multi-store support:
//   ?store_id=123   → filter for that specific store
//   ?store_id=      → all stores (consolidated)
//   (absent)        → all stores (consolidated, same as empty)
//
// Consolidated (no store filter, always all stores):
//   /dashboard/*  /vendas/*  /perguntas  /mensagens
//
// Per-store (respects ?store_id=):
//   /anuncios  /produtos  /pedidos  /clientes  /metricas
//   /analises/*  /publicidade  /concorrentes  /comparativos/*  /alertas/*
...
// Builds a WHERE clause fragment for store filtering.
// Returns { clause: 'AND store_id = $N', values: [storeId], nextIdx: N+1 }
// If storeId is falsy, returns empty clause (all stores).
function storeFilter(storeId, startIdx = 1) {
  if (!storeId) return { clause: '', values: [], nextIdx: startIdx };
  return {
    clause:  `AND store_id = $${startIdx}`,
    values:  [BigInt(storeId)],
    nextIdx: startIdx + 1,
  };
}

// ── Dashboard (consolidated — all stores) ─────────────────────────────────
  ...
  try {
    const data = await cached('kpis:summary', 30, async () => {
      const [today, perguntas, anuncios, alertas] = await Promise.all([
        ...
        pool.query(`SELECT COUNT(*) n FROM items WHERE status='active' AND available_quantity <= 5`),
      ]);
      return {
...
router.get('/dashboard/top-products', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT ml_id, title, sold_quantity, price, (sold_quantity * price) AS revenue, status
       FROM items WHERE status = 'active'
       ORDER BY sold_quantity DESC NULLS LAST LIMIT $1`,
      [limit]
    );
...
router.get('/dashboard/alerts', async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT ml_id, title, available_quantity, store_id
       FROM items WHERE status = 'active' AND available_quantity <= 5
       ORDER BY available_quantity ASC LIMIT 20`
    );
    res.json(rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
// ── Anúncios / Produtos (per-store) ───────────────────────────────────────

router.get('/anuncios', async (req, res) => {
  const { status = '', search = '', page = 1, limit = 20, store_id } = req.query;
  const sf = storeFilter(store_id, 5);
  const offset = (Number(page) - 1) * Number(limit);
  try {
    const [rows, summary] = await Promise.all([
      pool.query(
        `SELECT ml_id, title, price, available_quantity, sold_quantity, status, store_id, updated_at
         FROM items
         WHERE ($1 = '' OR status = $1)
           AND ($2 = '' OR title ILIKE '%'||$2||'%')
           ${sf.clause}
         ORDER BY updated_at DESC LIMIT $3 OFFSET $4`,
        [status, search, Number(limit), offset, ...sf.values]
      ),
      pool.query(
        `SELECT COUNT(*) total,
         ...
         FROM items WHERE TRUE ${sf.clause}`,
        sf.values
      ),
    ]);
...
router.get('/produtos', async (req, res) => {
  ...
  const sf = storeFilter(store_id, 1);
  try {
    const { rows } = await pool.query(
      `SELECT ml_id AS id, title, price, available_quantity AS stock,
              sold_quantity AS sold, status, store_id
       FROM items WHERE TRUE ${sf.clause}
       ORDER BY sold_quantity DESC NULLS LAST LIMIT 100`,
      sf.values
    );
    res.json({ products: rows });
  } catch (err) {
...
router.get('/produtos/performance', async (req, res) => {
  ...
  try {
    const [rows, totals] = await Promise.all([
      pool.query(
        `SELECT ml_id, title, visits, sold_quantity AS sales, position,
                CASE WHEN visits > 0 THEN ROUND((sold_quantity::numeric/visits)*100,2) ELSE 0 END conversion_rate
         FROM items WHERE status='active' ${sf.clause}
         ORDER BY sold_quantity DESC NULLS LAST LIMIT 50`,
        sf.values
      ),
      pool.query(
        `SELECT COALESCE(SUM(visits),0) total_visits,
                CASE WHEN SUM(visits)>0 THEN ROUND(SUM(sold_quantity)::numeric/SUM(visits)*100,2) ELSE 0 END conversion_rate
         FROM items WHERE status='active' ${sf.clause}`,
        sf.values
      ),
    ]);
    ...
    res.json({
      items: rows.rows,
      summary: { total_visits: fmt(s.total_visits), conversion_rate: fmt(s.conversion_rate), avg_ctr: null, roi: null },
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/pedidos', async (req, res) => {
  const { status = '', dateFrom, dateTo, store_id } = req.query;
  const sf = storeFilter(store_id, 4);
  try {
    const [rows, summary] = await Promise.all([
      pool.query(
        `SELECT ml_id AS id, buyer_nickname, title, total_amount, status, date_created, store_id
         FROM orders
         WHERE ($1 = '' OR status = $1)
        ...
        [status, dateFrom || null, dateTo || null, ...sf.values]
      ),
      pool.query(
        `SELECT
           ...
           COUNT(*) FILTER (WHERE status='delivered' AND date_created::date=CURRENT_DATE) entregues_hoje
         FROM orders WHERE TRUE ${sf.clause}`,
        sf.values
      ),
    ]);
    res.json({ results: rows.rows, summary: summary.rows[0] });
...
// ── Vendas (consolidated — all stores) ────────────────────────────────────
...
router.get('/vendas/diarias', async (req, res) => {
  ...
  try {
    const [rows, summary] = await Promise.all([
      pool.query(
        `SELECT date_created::date AS data, COUNT(*) pedidos, COALESCE(SUM(total_amount),0) bruto
...
// ── Perguntas (consolidated — all stores) ─────────────────────────────────
...
router.get('/perguntas', async (req, res) => {
  ...
  try {
    const [rows, summary] = await Promise.all([
      pool.query(
        `SELECT ml_id AS id, item_id, item_title, text, answer_text, status, date_created, store_id
         FROM questions WHERE ($1='' OR status=$1)
...
router.post('/perguntas/:id/responder', async (req, res) => {
  try {
    await pool.query(
      `UPDATE questions SET answer_text=$2, status='ANSWERED', answered_at=now(), updated_at=now() WHERE ml_id=$1`,
...
// ── Mensagens (consolidated — all stores) ─────────────────────────────────

router.get('/mensagens', async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT pack_id, buyer_nickname, last_message, unread, last_message_date, store_id
       FROM messages ORDER BY last_message_date DESC NULLS LAST LIMIT 50`
    );
...
router.get('/clientes', async (req, res) => {
  ...
  const sf = storeFilter(store_id, 2);
  try {
    const [rows, summary] = await Promise.all([
      pool.query(
        `SELECT buyer_nickname AS nickname, COUNT(*) total_orders,
                SUM(total_amount) total_spent, MAX(date_created) last_order_date
         FROM orders
         WHERE status!='cancelled' AND ($1='' OR buyer_nickname ILIKE '%'||$1||'%')
           ${sf.clause}
         GROUP BY buyer_nickname ORDER BY total_spent DESC NULLS LAST LIMIT 100`,
        [search, ...sf.values]
      ),
      pool.query(
        `SELECT
         ...
         FROM orders WHERE status!='cancelled' ${sf.clause}`,
        sf.values
      ),
    ]);
  ...
  try {
    const { rows } = await pool.query(
      `SELECT id, nickname, level_id, reputation_data, active_listings, monthly_revenue
       FROM stores WHERE ($1::bigint IS NULL OR id=$1::bigint) LIMIT 1`,
      [store_id || null]
    );
    if (!rows.length) return res.json({ level_id: null, metrics: {}, transactions: {} });
    const s = rows[0];
    const rep = s.reputation_data || {};
    res.json({ level_id: s.level_id, metrics: rep.metrics || {}, transactions: rep.transactions || {} });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
  ...
  try {
    const { rows } = await pool.query(
      `SELECT EXTRACT(HOUR FROM date_created)::int AS hour,
              COUNT(*) orders, COALESCE(SUM(total_amount),0) revenue
       FROM orders WHERE status!='cancelled' AND date_created>=CURRENT_DATE-30
         ${sf.clause}
       GROUP BY 1 ORDER BY 1`,
      sf.values
    );
  ...
  try {
    const { rows } = await pool.query(
      `SELECT EXTRACT(DOW FROM date_created)::int AS dow,
              COUNT(*) orders, COALESCE(SUM(total_amount),0) revenue
       FROM orders WHERE status!='cancelled' AND date_created>=CURRENT_DATE-90
         ${sf.clause}
       GROUP BY 1 ORDER BY 1`,
      sf.values
    );
    ...
    const [rows, summary] = await Promise.all([
      pool.query(
        `SELECT item_id, title, impressions, clicks, spend, revenue
         FROM ads_campaigns WHERE date=CURRENT_DATE ${sf.clause}
         ORDER BY spend DESC LIMIT 50`,
        sf.values
      ),
      pool.query(
        `SELECT COALESCE(SUM(spend) FILTER (WHERE date=CURRENT_DATE),0)   spend_today,
                COALESCE(SUM(revenue) FILTER (WHERE date=CURRENT_DATE),0) revenue_today,
                COUNT(*) FILTER (WHERE date=CURRENT_DATE)                 sponsored_count
         FROM ads_campaigns WHERE TRUE ${sf.clause}`,
        sf.values
      ),
    ]);
    const s = summary.rows[0];
    const acos = s.revenue_today > 0 ? ((s.spend_today/s.revenue_today)*100).toFixed(1) : null;
    res.json({
      campaigns: rows.rows,
...
// ── Comparativos (per-store) ───────────────────────────────────────────────

router.get('/comparativos/periodos', async (req, res) => {
  const { p1 = '30', p2 = '60', store_id } = req.query;
  const sf = storeFilter(store_id, 1);
  try {
    const [per1, per2] = await Promise.all([
      pool.query(
        `SELECT COUNT(*) pedidos, COALESCE(SUM(total_amount),0) receita,
         ...
         FROM orders WHERE date_created::date>=CURRENT_DATE-$1::int AND status!='cancelled' ${sf.clause}`,
        [Number(p1), ...sf.values]
      ),
      pool.query(
        `SELECT COUNT(*) pedidos, COALESCE(SUM(total_amount),0) receita,
                CASE WHEN COUNT(*)>0 THEN SUM(total_amount)/COUNT(*) ELSE 0 END ticket
         FROM orders WHERE date_created::date BETWEEN CURRENT_DATE-$1::int AND CURRENT_DATE-$2::int
           AND status!='cancelled' ${sf.clause}`,
        [Number(p2), Number(p1), ...sf.values]
      ),
    ]);
...
router.get('/comparativos/evolucao', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT date_created::date AS data, COUNT(*) pedidos, COALESCE(SUM(total_amount),0) receita
       FROM orders WHERE date_created>=CURRENT_DATE-$1::int AND status!='cancelled'
         ${sf.clause}
       GROUP BY 1 ORDER BY 1`,
      [Math.min(Number(days), 90), ...sf.values]
    );
    res.json({ rows });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.get('/comparativos/curva-abc', async (req, res) => {
  ...
  try {
    const { rows: all } = await pool.query(
      `SELECT ml_id, title, sold_quantity*price AS revenue, sold_quantity
       FROM items WHERE status='active' AND sold_quantity>0 ${sf.clause}
...
router.get('/alertas/reposicao', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT ml_id, title, available_quantity AS stock, store_id,
              COALESCE(sold_quantity::float/30,0) AS daily_sales
       FROM items WHERE status='active' AND available_quantity<=15 ${sf.clause}
    ...
    const [rows, summary] = await Promise.all([
      pool.query(
        `SELECT ml_id AS order_id, buyer_nickname, title, total_amount AS amount,
                cancelled_by, cancel_reason AS reason, date_closed AS date, store_id
         FROM orders WHERE status='cancelled' ${sf.clause}
         ORDER BY date_closed DESC NULLS LAST LIMIT $${sf.nextIdx}`,
        [...sf.values, Number(limit)]
      ),
      pool.query(
        `SELECT COUNT(*) total, COUNT(*) FILTER(WHERE date_closed::date=CURRENT_DATE) hoje,
...
router.get('/alertas/devolucoes', async (req, res) => {
  ...
  try {
    const [rows, summary] = await Promise.all([
      pool.query(
        `SELECT r.id, r.order_id, r.title, r.reason, r.amount, r.status, r.date, r.store_id
         FROM returns r WHERE TRUE ${sf.clause}
         ORDER BY r.date DESC NULLS LAST LIMIT 100`,
        sf.values
      ),
      pool.query(
        `SELECT COUNT(*) FILTER(WHERE status='analysis') in_analysis,
                COUNT(*) FILTER(WHERE status='approved')  approved,
                COUNT(*) FILTER(WHERE status='rejected')  rejected,
                COALESCE(SUM(amount),0)                   total_value
         FROM returns WHERE TRUE ${sf.clause}`,
        sf.values
      ),
    ]);
    res.json({ items: rows.rows, summary: summary.rows[0] });
...
router.get('/alertas/anuncios-problema', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT ml_id, title, status, available_quantity, store_id, updated_at
       FROM items
       WHERE (status IN ('paused','closed') OR (status='active' AND available_quantity=0))
         ${sf.clause}
    ...
    res.json({
      items: rows.map(r => ({
        ...r,
        problema: r.status==='paused' ? 'Pausado' : r.status==='closed' ? 'Encerrado' : 'Sem estoque',
      })),
  ...
  try {
    const { rows } = await pool.query(
      `SELECT topic, resource, store_id, status, error, received_at, processed_at
       FROM webhook_logs WHERE ($1='' OR topic=$1)
