...
//
// Camada de análise ANTES de qualquer LLM (fase 2, futura, não implementada
// nesta tarefa — ver decisions.md/todo.md): aqui só cálculo determinístico.
// ═══════════════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    produtos.forEach(s => {
      if (s.classificacao === 'PREJUIZO') {
        ...
        // Simulação de +5% no preço (spec do usuário): tarifa e imposto são
        // % do valor da venda, então escalam com o preço; custo do produto e
        // frete são fixos por unidade, não escalam. Volume assumido CONSTANTE
        // — premissa explícita, não é previsão.
        const impacto = 0.05 * (s.faturamento - s.imposto - s.tarifa);
        acoes.push({
          ...
          acao: 'Testar aumento de preço de 5%.',
          impacto_estimado: Math.max(0, impacto),
